...
func VerifyTransaction(Tx *tx.Transaction, ledger *ledger.Ledger, TxPool []*tx.Transaction) error {
	...
	err = CheckTransactionCcntmracts(Tx)
	if err != nil {
		return err
	}
...
//remove the invalidate transaction from process ccntmext and tell the caller which should be removed.
