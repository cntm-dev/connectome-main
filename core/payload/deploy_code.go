...
type DeployCode struct {
	Code []byte
	//modify for define ccntmract type
	VmType      byte
	Name        string
	Version     string
	Author      string
...
func validateDeployCode(dep *DeployCode) error {
	...
	if len(dep.Author) > 252 {
		return errors.NewErr("[author] version too lcntm!")
	}

	if len(dep.Email) > 252 {
		return errors.NewErr("[author] emailPtr too lcntm!")
	}

	if len(dep.Description) > 65536 {
		return errors.NewErr("[descPtr] emailPtr too lcntm!")
	}
