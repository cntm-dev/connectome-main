...
// Ledger - the struct for cntmDNA ledger
type Ledger struct {
...
func GetMinerAddress(miners []*crypto.PubKey) (Uint160,error) {
	...
	var err error
	if len(miners) > 1 {
		temp, err = ccntmract.CreateMultiSigRedeemScript(len(miners) - (len(miners) - 1) / 3, miners)
		if err != nil {
			return Uint160{}, NewDetailErr(err, ErrNoCode, "[Ledger],GetMinerAddress failed with CreateMultiSigRedeemScript.")
		}
	} else {
		temp, err = ccntmract.CreateSignatureRedeemScript(miners[0])
		if err != nil {
			return Uint160{}, NewDetailErr(err, ErrNoCode, "[Ledger],GetMinerAddress failed with CreateMultiSigRedeemScript.")
		}
	}
	...
	return codehash,nil
	//return Ccntmract.CreateMultiSigRedeemScript(miners.Length - (miners.Length - 1) / 3, miners).ToScriptHash();
}
