...
//check weather the transaction ccntmains the doubleSpend.
...
func GetBookKeeperAddress(bookKeepers []*crypto.PubKey) (Uint160, error) {
	...
	var err error
	if len(bookKeepers) > 1 {
		temp, err = ccntmract.CreateMultiSigRedeemScript(len(bookKeepers)-(len(bookKeepers)-1)/3, bookKeepers)
		if err != nil {
			return Uint160{}, NewDetailErr(err, ErrNoCode, "[Ledger],GetBookKeeperAddress failed with CreateMultiSigRedeemScript.")
		}
	} else {
		temp, err = ccntmract.CreateSignatureRedeemScript(bookKeepers[0])
		if err != nil {
			return Uint160{}, NewDetailErr(err, ErrNoCode, "[Ledger],GetBookKeeperAddress failed with CreateMultiSigRedeemScript.")
		}
	}
