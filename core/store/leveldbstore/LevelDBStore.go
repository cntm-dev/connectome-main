...
func (bd *LevelDBStore) GetCcntmract(hash []byte) ([]byte, error) {
...
func (bd *LevelDBStore) persistBlocks(ledger *Ledger) {
	...
	for !bd.disposed {
		...
		if !ok {
			log.Warn("[persistBlocks]: warn, block_cache not ccntmain key hash.")
			break
		}
		bd.persist(block)
...
func (bd *LevelDBStore) CcntmainsUnspent(txid Uint256, index uint16) (bool, error) {
	unspentprefix := []byte{byte(IX_Unspent)}
