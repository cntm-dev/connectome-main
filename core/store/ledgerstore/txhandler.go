...
//			Admin:      registerAsset.Ccntmroller,
//			Issuer:     registerAsset.Ccntmroller,
...
func (this *StateStore) HandleInvokeTransaction(stateBatch *statestore.StateBatch, tx *types.Transaction, block *types.Block, eventStore common.IEventStore) error {
	...
	//txHash := tx.Hash()
	//ccntmrState, err := this.TryGet(ST_Ccntmract, invoke.CodeHash.ToArray())
	//if err != nil {
	//	return fmt.Errorf("TryGet ccntmract error %s", err)
	//}
	//if ccntmrState == nil {
	//	//ledgerevent.PushSmartCodeEvent(tx.Hash(), 0, INVOKE_TRANSACTION, "Ccntmract not found!")
	//	return nil
	//}
	//
	//ccntmract := ccntmrState.Value.(*CcntmractState)
	//stateMachine := service.NewStateMachine(this, types.Application, block)
	//smc, err := smartccntmract.NewSmartCcntmract(
	//	&smartccntmract.Ccntmext{
	...
	//	})
	//if err != nil {
	//	return fmt.Errorf("NewSmartCcntmract error %s", err)
	//}
	//_, err = smc.InvokeCcntmract()
	//if err != nil {
	//	//ledgerevent.PushSmartCodeEvent(txHash, SMARTCODE_ERROR, INVOKE_TRANSACTION, err)
	//	return fmt.Errorf("InvokeCcntmract error %s", err)
	//}
