...
import (
	...
	sc "github.com/Ontology/smartccntmract"
...
func (bd *ChainStore) GetCcntmract(hash Uint160) (*states.CcntmractState, error) {
	ccntmract := new(states.CcntmractState)
	data, err := bd.st.Get(append([]byte{byte(ST_Ccntmract)}, hash.ToArray()...))
	if err != nil {
		return nil, err
	}
	...
	for _, t := range b.Transactions {
		...
		case tx.BookKeeper:
			...
			case payload.BookKeeperAction_ADD:
				if crypto.CcntmainPubKey(bk.PubKey, bookKeeper.NextBookKeeper) < 0 {
			...
			case payload.BookKeeperAction_SUB:
				index := crypto.CcntmainPubKey(bk.PubKey, bookKeeper.NextBookKeeper)
				if index >= 0 {
		...
		case tx.Deploy:
			...
			if err := stateStore.TryGetOrAdd(ST_Ccntmract, codeHash.ToArray(), &states.CcntmractState{
				Code:        deploy.Code,
				...
				log.Error("[persist] TryAdd ST_Ccntmract error:", err)
				return err
			}
		case tx.Invoke:
			invoke := t.Payload.(*payload.InvokeCode)
			cs, err := stateStore.TryGet(ST_Ccntmract, invoke.CodeHash.ToArray())
			if err != nil {
				log.Error("[persist] TryGet ST_Ccntmract error:", err)
				return err
			}
			if cs == nil {
				event.PushSmartCodeEvent(t.Hash(), 0, INVOKE_TRANSACTION, "Ccntmract not found!")
				ccntminue
			}
			...
			smc, err := sc.NewSmartCcntmract(&sc.Ccntmext{
				...
				ReturnType:     ccntmract.Code.ReturnType,
			})
			if err != nil {
				log.Error("[persist] NewSmartCcntmract error:", err)
				return err
			}
			ret, err := smc.InvokeCcntmract()
			if err != nil {
				log.Error("[persist] InvokeCcntmract error:", err)
...
func (bd *ChainStore) CcntmainsUnspent(txid Uint256, index uint16) (bool, error) {
	unspentPrefix := []byte{byte(ST_Coin)}
...
func (bd *ChainStore) GetIdentity(cntmId []byte) ([]byte, error) {
	idPrefix := []byte{byte(ST_Identity)}
	idKey := append(idPrefix, cntmId...)
	return bd.st.Get(idKey)
}

func (bd *ChainStore) SetIdentity(cntmId, ddo []byte) error {
	idPrefix := []byte{byte(ST_Identity)}
	idKey := append(idPrefix, cntmId...)
...
func (bd *ChainStore) GetVotesAndEnrollments(txs []*tx.Transaction) ([]*states.VoteState, []*crypto.PubKey, error) {
	...
	for k, v := range voteStates {
		...
		for _, b := range account.Balances {
			if b.AssetId.CompareTo(tx.cntmTokenID) == 0 {
				v.Count = b.Amount
			}
		}
		if s, ok := result[k]; ok {
...
func (bd *ChainStore) getBlockTransactionResult(txs []*tx.Transaction) (map[Uint160]Fixed64,
	...
	for _, t := range txs {
		for _, i := range t.UTXOInputs {
			if i.ReferTxID.CompareTo(tx.cntmTokenID) != 0 {
				ccntminue
			}
		...
		for _, o := range t.Outputs {
			if o.AssetID.CompareTo(tx.cntmTokenID) != 0 {
				ccntminue
			}
