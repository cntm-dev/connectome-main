...
func (bd *ChainStore) GetCcntmract(hash []byte) ([]byte, error) {
	prefix := []byte{byte(DATA_Ccntmract)}
	...
	log.Debug("GetCcntmract Data: ", bData)
...
func (bd *ChainStore) ccntmainsBlock(hash Uint256) bool {
...
func (bd *ChainStore) persistBlocks(ledger *Ledger) {
	...
	for !bd.disposed {
		...
		if !ok {
			log.Warn("[persistBlocks]: warn, blockCache not ccntmain key hash.")
			break
		}
		bd.persist(block)
