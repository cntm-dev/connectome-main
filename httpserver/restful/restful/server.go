...
import (
	...
	"github.com/conntectome/cntm/http/base/common"
	berr "github.com/conntectome/cntm/http/base/error"
...
const (
	...
	GET_CCNTMRACT_STATE    = "/api/v1/contract/:hash"
	...
	GET_UNBOUNDCNTG        = "/api/v1/unboundong/:addr"
	GET_GRANTCNTG          = "/api/v1/grantong/:addr"
...
func (this *restServer) registryMethod() {
	...
	getMethodMap := map[string]Action{
		...
		GET_CCNTMRACT_STATE:    {name: "getcontract", handler: rest.GetContractState},
		...
		GET_UNBOUNDCNTG:        {name: "getunboundong", handler: rest.GetUnboundCntg},
		GET_GRANTCNTG:          {name: "getgrantong", handler: rest.GetGrantCntg},
...
func (this *restServer) getPath(url string) string {
	...
	if strings.Contains(url, strings.TrimRight(GET_BLK_TXS_BY_HEIGHT, ":height")) {
		...
		return GET_TX
	} else if strings.Contains(url, strings.TrimRight(GET_CCNTMRACT_STATE, ":hash")) {
	...
	} else if strings.Contains(url, strings.TrimRight(GET_UNBOUNDCNTG, ":addr")) {
		return GET_UNBOUNDCNTG
	} else if strings.Contains(url, strings.TrimRight(GET_GRANTCNTG, ":addr")) {
		return GET_GRANTCNTG
