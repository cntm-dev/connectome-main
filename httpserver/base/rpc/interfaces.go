...
func SendRawTransaction(params []interface{}) map[string]interface{} {
	...
	case string:
		...
		if txn.TxType == types.InvokeCntm || txn.TxType == types.Deploy || txn.TxType == types.InvokeWasm {
			if len(params) > 1 {
...
func GetContractState(params []interface{}) map[string]interface{} {
	...
	case string:
		...
		if err != nil {
			return responsePack(berr.UNKNOWN_CCNTMRACT, berr.ErrMap[berr.UNKNOWN_CCNTMRACT])
		}
		contract = c
	default:
		return responsePack(berr.INVALID_PARAMS, "")
...
// get unbound cntg of address
func GetUnboundCntg(params []interface{}) map[string]interface{} {
	if len(params) < 1 {
		return responsePack(berr.INVALID_PARAMS, "")
	}
	str, ok := params[0].(string)
	...
	fromAddr := utils.CntmContractAddress
	rsp, err := bcomn.GetAllowance("cntg", fromAddr, toAddr)
	if err != nil {
		return responsePack(berr.INVALID_PARAMS, "")
	}
	return responseSuccess(rsp)
...
// get grant cntg of address
func GetGrantCntg(params []interface{}) map[string]interface{} {
	if len(params) < 1 {
		return responsePack(berr.INVALID_PARAMS, "")
	}
	str, ok := params[0].(string)
	...
	rsp, err := bcomn.GetGrantCntg(toAddr)
	if err != nil {
		return responsePack(berr.INTERNAL_ERROR, "")
	}
	return responseSuccess(rsp)
