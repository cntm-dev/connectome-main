...
func SendRawTransaction(cmd map[string]interface{}) map[string]interface{} {
	...
	if txn.TxType == types.InvokeCntm || txn.TxType == types.InvokeWasm || txn.TxType == types.Deploy {
...
//get unbound cntg
func GetUnboundCntg(cmd map[string]interface{}) map[string]interface{} {
	resp := ResponsePack(berr.SUCCESS)
	...
	bcomn.GetGrantCntg(toAddr)
	resp["Result"] = rsp
	return resp
}

//get grant cntg
func GetGrantCntg(cmd map[string]interface{}) map[string]interface{} {
	resp := ResponsePack(berr.SUCCESS)
	toAddrStr, ok := cmd["Addr"].(string)
	if !ok {
		return ResponsePack(berr.INVALID_PARAMS)
