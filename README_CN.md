...
注意：当你想搭建一个私网去运行DBFT或者VBFT模式的cntmology时，你必须要使用 --config参数去指定一个具体的配置文件，并且要使用 --networkid去定义一个
网络标识（不能等于1/2/3），否则，cntmology节点将会自动连接到主网上。
...
了解更多请运行 `./cntmology --help`，你也可以参考[Ontology CLI 用户指引](./docs/specifications/cli_user_guide_CN.md)获得更多信息。
...
2. 运行cntmology镜像
    ...
    - 如果需要使镜像运行时接受交互式键盘输入，则使用`docker -ti run cntmio/cntmology`命令启动镜像即可；
