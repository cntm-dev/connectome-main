...
func walletAction(c *cli.Ccntmext) (err error) {
	if c.NumFlags() == 0 {
	...
	if name != "" && passwd != "" {
		if create {
			...
			if name != "" && passwd != "" {
				...
				signatureRedeemScript, _ := ccntmract.CreateSignatureRedeemScript(account.PubKey())
...
func NewCommand() *cli.Command {
	return &cli.Command{
		...
		Description: "With nodectl wallet, you could ccntmrol your asset.",
		ArgsUsage:   "[args]",
		Flags: []cli.Flag{
			cli.BoolFlag{
				Name:  "create, c",
