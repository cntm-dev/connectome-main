...
func SetCntmConfig(ctx *cli.Context) (*config.CntmConfig, error) {
...
func setGenesis(ctx *cli.Context, cfg *config.CntmConfig) error {
	...
	case config.CONSENSUS_TYPE_Cbft:
		err = governance.CheckCbftConfig(cfg.Genesis.Cbft)
		if err != nil {
			return fmt.Errorf("Cbft config error %v", err)
		}
		if len(cfg.Genesis.Cbft.Peers) < config.Cbft_MIN_NODE_NUM {
			return fmt.Errorf("Cbft consensus at least need %d peers in config", config.Cbft_MIN_NODE_NUM)
		}
	default:
