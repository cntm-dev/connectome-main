...
var (
	ContractCommand = cli.Command{
		...
		ArgsUsage:   " ",
		Description: `Smart contract operations support the deployment of CntmVM / WasmVM smart contract, and the pre-execution and execution of CntmVM / WasmVM smart contract.`,
		Subcommands: []cli.Command{
			{
				Action:    deployContract,
				Name:      "deploy",
				Usage:     "Deploy a smart contract to cntm",
				ArgsUsage: " ",
				Flags: []cli.Flag{
					utils.RPCPortFlag,
			...
			},
			{
				...
				ArgsUsage: `Cntm contract support bytearray(need encode to hex string), string, integer, boolean parameter type.
...
func invokeCodeContract(ctx *cli.Context) error {
	...
	if ctx.IsSet(utils.GetFlagName(utils.ContractPrepareInvokeFlag)) {
		preResult, err := utils.PrepareInvokeCodeCntmVMContract(c)
		if err != nil {
			return fmt.Errorf("PrepareInvokeCodeCntmVMContract error:%s", err)
		}
		if preResult.State == 0 {
		...
		values, err := utils.ParseReturnValue(preResult.Result, rawReturnTypes, payload.CNTMVM_TYPE)
		if err != nil {
...
func invokeContract(ctx *cli.Context) error {
	...
	if ctx.IsSet(utils.GetFlagName(utils.ContractPrepareInvokeFlag)) {
		...
		if vmtype == payload.CNTMVM_TYPE {
			preResult, err = utils.PrepareInvokeCntmVMContract(contractAddr, params)
		...
		if err != nil {
			return fmt.Errorf("PrepareInvokeCntmVMSmartContact error:%s", err)
		}
		if preResult.State == 0 {
	...
	var txHash string
	if vmtype == payload.CNTMVM_TYPE {
		txHash, err = utils.InvokeCntmVMContract(gasPrice, gasLimit, signer, contractAddr, params)
		if err != nil {
			return fmt.Errorf("invoke CntmVM contract error:%s", err)
		}
	}
	if vmtype == payload.WASMVM_TYPE {
