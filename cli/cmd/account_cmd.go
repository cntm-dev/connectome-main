...
import (
	...
	"github.com/conntectome/cntm/cmd/common"
	"github.com/conntectome/cntm/cmd/utils"
	"github.com/conntectome/cntm/common/password"
...
You can use ./Cntm account --help command to view help information of wallet management command.`,
   ...
   Cntm support three type of key: ecdsa, sm2 and ed25519, and support 224、256、384、521 bits length of key in ecdsa, but only support 256 bits length of key in sm2 and ed25519.
   Cntm support multiple signature scheme.
...
func accountCreate(ctx *cli.Context) error {
	...
	if ctx.Bool(utils.IdentityFlag.Name) {
		...
		if err != nil {
			return fmt.Errorf("error creating CNTM ID: %s", err)
		}
		wd.AddIdentity(id)
		...
		PrintInfoMsg("CNTM ID created: %s", id.ID)
