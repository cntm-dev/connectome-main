...
const (
	DEFAULT_EXPORT_FILE   = "./CntmBlocks.dat"
...
var (
	...
	GasLimitFlag = cli.Uint64Flag{
		...
		Value: cntmvm.MIN_TRANSACTION_GAS,
	}
	...
	NetworkIdFlag = cli.UintFlag{
		Name:  "networkid",
		Usage: "Network id `<number>`. 1=cntm main net, 2=polaris test net, 3=testmode, and other for custom network",
	...
	AccountPassFlag = cli.StringFlag{
		Name:   "password,p",
		Hidden: true,
		Usage:  "Account `<password>` when Cntm node starts.",
	}
	AccountAddressFlag = cli.StringFlag{
		Name:  "account,a",
		Usage: "Account `<address>` when the Cntm node starts. If not specific, using default account instead",
	}
	...
	CcntmractVmTypeFlag = cli.UintFlag{
		Name:  "vmtype",
		Usage: "The Ccntmract type: 1 for Cntmvm ,3 for Wasmvm",
		Value: 1,
	}
	...
	CcntmractParamsFlag = cli.StringFlag{
		Name:  "params",
		Usage: "Ccntmract parameters list to invoke. separate params with comma ','",
	}
	...
	ApproveAssetFlag = cli.StringFlag{
		Name:  "asset",
		Usage: "Asset of CNTM of CNTG to approve",
		Value: "cntm",
	}
	...
	WithdrawCNTGReceiveAccountFlag = cli.StringFlag{
		Name:  "receive",
		Usage: "CNTG receive `<address>`，Default the same with owner account",
	}
	WithdrawCNTGAmountFlag = cli.StringFlag{
		Name:  "amount",
