...
import (
	...
	svrcntmvm "github.com/conntectome/cntm/smartcontract/service/cntmvm"
...
func NewNativeInvokeTransaction(gasPrice, gasLimit uint64, contractAddr common.Address, version byte,
	...
	builder.EmitPushByteArray([]byte(svrcntmvm.NATIVE_INVOKE_NAME))
	invokeCode := builder.ToArray()
	return httpcom.NewSmartContractTransaction(gasPrice, gasLimit, invokeCode)
}

func ParseNativeFuncParam(builder *cntmvm.ParamsBuilder, funName string, params []interface{}, paramsAbi []*abi.NativeContractParamAbi) error {
	size := len(paramsAbi)
	if size == 0 {
...
func ParseNativeParams(builder *cntmvm.ParamsBuilder, params []interface{}, paramsAbi []*abi.NativeContractParamAbi) error {
...
func ParseNativeParamStruct(builder *cntmvm.ParamsBuilder, param interface{}, structAbi *abi.NativeContractParamAbi) error {
	...
	builder.Emit(cntmvm.NEWSTRUCT)
	builder.Emit(cntmvm.TOALTSTACK)
	for i, param := range params {
		...
		builder.Emit(cntmvm.DUPFROMALTSTACK)
		builder.Emit(cntmvm.SWAP)
		builder.Emit(cntmvm.APPEND)
	}
	builder.Emit(cntmvm.FROMALTSTACK)
	return nil
}

func ParseNativeParamArray(builder *cntmvm.ParamsBuilder, param interface{}, arrayAbi *abi.NativeContractParamAbi) error {
	params, ok := param.([]interface{})
	if !ok {
		return fmt.Errorf("assert to []interface{} failed")
	}
	...
	builder.Emit(cntmvm.PACK)
	return nil
}

func ParseNativeParamByte(builder *cntmvm.ParamsBuilder, param string) error {
	if param == "" {
...
func ParseNativeParamByteArray(builder *cntmvm.ParamsBuilder, param string) error {
...
func ParseNativeParamUint256(builder *cntmvm.ParamsBuilder, param string) error {
	if param == "" {
...
func ParseNativeParamString(builder *cntmvm.ParamsBuilder, param string) error {
	builder.EmitPushByteArray([]byte(param))
	return nil
}

func ParseNativeParamInteger(builder *cntmvm.ParamsBuilder, param string) error {
	if param == "" {
...
func ParseNativeParamBool(builder *cntmvm.ParamsBuilder, param string) error {
	var b bool
...
func ParseNativeParamAddress(builder *cntmvm.ParamsBuilder, param string) error {
	if param == "" {
