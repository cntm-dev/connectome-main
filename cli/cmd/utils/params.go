...
func ParseReturnValue(rawValue interface{}, rawReturnTypeStr string, vmtype payload.VmType) ([]interface{}, error) {
	...
	if vmtype == payload.CNTMVM_TYPE {
		return parseReturnCntmValueArray(rawValues, returnTypes)
	} else {
...
func parseReturnCntmValueArray(rawValues []interface{}, returnTypes []interface{}) ([]interface{}, error) {
	values := make([]interface{}, 0)
	for i := 0; i < len(rawValues); i++ {
		rawValue := rawValues[i]
		if i == len(returnTypes) {
		...
		case string:
			...
			switch strings.ToLower(vType) {
			case PARAM_TYPE_BYTE_ARRAY:
				value, err = ParseCntmVMContractReturnTypeByteArray(v)
			case PARAM_TYPE_STRING:
				value, err = ParseCntmVMContractReturnTypeString(v)
			case PARAM_TYPE_INTEGER:
				value, err = ParseCntmVMContractReturnTypeInteger(v)
			case PARAM_TYPE_BOOLEAN:
				value, err = ParseCntmVMContractReturnTypeBool(v)
			default:
				return nil, fmt.Errorf("unknown return type:%s", v)
			}
			values = append(values, value)
		...
		case []interface{}:
			...
			values, err := parseReturnCntmValueArray(v, valueTypes)
			if err != nil {
				return nil, fmt.Errorf("parese return values:%+v types:%s error:%s", values, valueType, err)
			}
		default:
...
//	},
//}
type CntmVMInvokeParam struct {
	Type  string
	Value interface{} //string or []*CntmVMInvokeParam
}

//ParseCntmVMInvokeParams parse params to []interface, rawParams is array of CntmVMInvokeParam
func ParseCntmVMInvokeParams(rawParams []interface{}) ([]interface{}, error) {
	if len(rawParams) == 0 {
		return nil, nil
	}
	params := make([]interface{}, 0)
	for _, rawParam := range rawParams {
		...
		case string:
			...
			params = append(params, param)
		case []interface{}:
			ps, err := ParseCntmVMInvokeParams(pv)
			if err != nil {
				return nil, err
			}
			if len(ps) > 0 {
