...
func sampleTransaction(issuer, admin *client.Account, index int64,buf []byte, nosign bool) *transaction.Transaction {
	...
	// generate ccntmrollerPGM
	ccntmrollerPGM, _ := ccntmract.CreateSignatureCcntmract(admin.PubKey())
	// generate transaction
	ammount := Fixed64(1000)
	tx, _ := transaction.NewRegisterAssetTransaction(a, ammount, issuer.PubKey(), ccntmrollerPGM.ProgramHash)
	if nosign {
		return tx
	}
	// generate signature
	...
	// get CcntmractCcntmext Programs & set into transaction
