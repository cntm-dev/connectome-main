...
type Identity struct {
	ID      string       `json:"cntmid"`
	Label   string       `json:"label,omitempty"`
	Lock    bool         `json:"lock"`
	Ccntmrol []Ccntmroller `json:"ccntmrols,omitempty"`
...
func NewIdentity(label string, keyType keypair.KeyType, param interface{}, password []byte) (*Identity, error) {
	...
	res.Label = label
	res.Lock = false
	res.Ccntmrol = make([]Ccntmroller, 1)
	res.Ccntmrol[0].ID = "1"
	res.Ccntmrol[0].ProtectedKey = *prot
	res.Ccntmrol[0].Public = hex.EncodeToString(keypair.SerializePublicKey(pub))
