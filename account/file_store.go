...
func (cs *FileStore) readDB() ([]byte, error) {
	...
	if cs.file != nil {
		...
		return nil, cntmErrors.NewDetailErr(errors.New("[readDB] file handle is nil"), cntmErrors.ErrNoCode, "")
	}
}
...
func (cs *FileStore) LoadStoredData(name string) ([]byte, error) {
	...
	return nil, cntmErrors.NewDetailErr(errors.New("Can't find the key: "+name), cntmErrors.ErrNoCode, "")
}
