...
func (s *BusynessActor) Receive(ccntmext actor.Ccntmext) {
	...
	case *RunMsg:
		...
		setEvent := &eventhub.Event{Topic:"SETTOPIC",Publisher:ccntmext.Self(),Message:setPrivMsg,Policy:eventhub.PUBLISH_POLICY_ALL}
		...
		for i := 0; i < loop; i++ {
			...
			s.Datas[idx] = data
			sigEvent := &eventhub.Event{Topic:"SIGTOPIC",Publisher:ccntmext.Self(),Message:sigMsg,Policy:eventhub.PUBLISH_POLICY_ROUNDROBIN}
			eventhub.GlobalEventHub.Publish(sigEvent)
			//signActor.Request(sigMsg,ccntmext.Self())
		}
	...
	case *SignResponse:
		...
		//vfActor.Request(vfr,ccntmext.Self())

		vrfEvent := &eventhub.Event{Topic:"VERIFYTOPIC",Publisher:ccntmext.Self(),Message:vfr,Policy:eventhub.PUBLISH_POLICY_ROUNDROBIN}
