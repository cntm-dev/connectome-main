...
func (f *Future) ccntminueWith(ccntminuation func(res interface{}, err error)) {
	f.cond.L.Lock()
	defer f.cond.L.Unlock() //use defer as the ccntminuation could blow up
	if f.done {
		ccntminuation(f.result, f.err)
	} else {
		f.completions = append(f.completions, ccntminuation)
	}
}
