/*
 * Copyright (C) 2021 The cntmology Authors
...
func (self *StateDB) CommitToCacheDB() error {
	for addr := range self.Suicided {
		...
		err := self.cacheDB.CleanCcntmractStorageData(comm.Address(addr))
		if err != nil {
			return err
		}
	}
...
func genKey(ccntmract common.Address, key common.Hash) []byte {
	var result []byte
...
func (self *StateDB) GetState(ccntmract common.Address, key common.Hash) common.Hash {
	val, err := self.cacheDB.Get(genKey(ccntmract, key))
	if err != nil {
...
func (self *StateDB) SetState(ccntmract common.Address, key, value common.Hash) {
	self.cacheDB.Put(genKey(ccntmract, key), value[:])
}
...
func (self *EthAccount) IsEmptyCcntmract() bool {
