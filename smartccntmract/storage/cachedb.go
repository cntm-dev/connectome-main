...
// NewCacheDB return a new ccntmract cache
...
func (self *CacheDB) GetCcntmract(addr comm.Address) (*payload.DeployCode, error) {
	value, err := self.get(common.ST_CcntmRACT, addr[:])
	if err != nil {
		return nil, err
	}
	...
	if err := ccntmract.Deserialization(comm.NewZeroCopySource(value)); err != nil {
		return nil, err
	}
...
func (self *CacheDB) DeleteCcntmract(address comm.Address) {
	self.delete(common.ST_CcntmRACT, address[:])
}
