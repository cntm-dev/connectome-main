...
func(this *WasmVmService)blockChainGetCcntmract(engine *exec.ExecutionEngine)(bool,error) {
	vm := engine.GetVM()
	envCall := vm.GetEnvCall()
	params := envCall.GetParams()
	if len(params) != 1 {
		return false, errors.NewErr("[blockChainGetCcntmract]parameter count error ")
	}
	...
	item, err := this.Store.GetCcntmractState(address); if err != nil {
		return false,errors.NewDetailErr(err, errors.ErrNoCode, "[blockChainGetCcntmract] GetAsset error!")
	}
