...
func (self *Runtime) Notify(proc *exec.Process, ptr uint32, len uint32) {
	...
	notify := &event.NotifyEventInfo{self.Service.CcntmextRef.CurrentCcntmext().CcntmractAddress, bs}
...
func (self *Runtime) CallCcntmract(proc *exec.Process, ccntmractAddr uint32, inputPtr uint32, inputLen uint32) uint32 {
...
func NewHostModule(host *Runtime) *wasm.Module {
	...
	m.FunctionIndexSpace = []wasm.Function{
		...
		{ //15
			...
			Host: reflect.ValueOf(host.CallCcntmract),
			Body: &wasm.FunctionBody{}, // create a dummy wasm body (the actual value will be taken from Host.)
		},
		{ //16
		...
		{ //19
			...
			Host: reflect.ValueOf(host.CcntmractCreate),
			Body: &wasm.FunctionBody{}, // create a dummy wasm body (the actual value will be taken from Host.)
		},
		{ //20
			Sig:  &m.Types.Entries[9],
			Host: reflect.ValueOf(host.CcntmractMigrate),
			Body: &wasm.FunctionBody{}, // create a dummy wasm body (the actual value will be taken from Host.)
		},
		{ //21
			Sig:  &m.Types.Entries[10],
			Host: reflect.ValueOf(host.CcntmractDelete),
			Body: &wasm.FunctionBody{}, // create a dummy wasm body (the actual value will be taken from Host.)
		},
		{ //22
			Sig:  &m.Types.Entries[4],
