...
func NewStateMachine(dbCache store.IStateStore) *StateMachine {
	...
	stateMachine.StateReader.Register("Neo.Storage.GetCcntmext", stateMachine.StorageGetCcntmext)
...
func (s *StateMachine) CcntmractCreate(engine *vm.ExecutionEngine) (bool, error) {
	...
	state, err := s.CloneCache.GetOrAdd(store.ST_Ccntmract, codeHash.ToArray(), ccntmractState); if err != nil {
