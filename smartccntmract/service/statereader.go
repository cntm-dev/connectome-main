...
func (s *StateReader) CheckWitnessHash(engine *vm.ExecutionEngine, programHash common.Uint160) (bool, error) {
	hashForVerifying, err := engine.GetCodeCcntmainer().(signature.SignableData).GetProgramHashes()
	if err != nil {
		return false, err
	}
	return ccntmains(hashForVerifying, programHash), nil
}
...
func (s *StateReader) GetCcntmract(e *vm.ExecutionEngine) (bool, error) {
	...
	item, err := ledger.DefaultLedger.Store.GetCcntmract(hash)
	if err != nil {
