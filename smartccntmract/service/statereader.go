...
func (s *StateReader) RuntimeNotify(e *vm.ExecutionEngine) (bool, error) {
	...
	txid := tran.Hash()
	s.Notifications = append(s.Notifications, &event.NotifyEventInfo{Ccntmainer: txid, CodeHash: hash, States: ConvertReturnTypes(item)})
	event.PushSmartCodeEvent(tran.Hash(), 0, Notify, event.NotifyEventArgs{Ccntmainer: txid, CodeHash: hash, States: item})
	return true, nil
}
...
func (s *StateReader) TransactionGetInputs(e *vm.ExecutionEngine) (bool, error) {
	...
	txn, ok := d.(*types.Transaction)
	if ok == false {
		return false, errors.NewErr("[TransactionGetInputs] Wrcntm type!")
	}
...
func (s *StateReader) TransactionGetOutputs(e *vm.ExecutionEngine) (bool, error) {
	...
	txn, ok := d.(*types.Transaction)
	if ok == false {
		return false, errors.NewErr("[TransactionGetOutputs] Wrcntm type!")
	}
	outputs := txn.Outputs
...
func (s *StateReader) InputGetIndex(e *vm.ExecutionEngine) (bool, error) {
	...
	input, ok := d.(*utxo.UTXOTxInput)
	if ok == false {
		return false, errors.NewErr("[InputGetIndex] Wrcntm type!")
	}
