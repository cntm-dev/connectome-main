...
import (
	...
	"github.com/cntmio/cntmology/smartccntmract/service/evm/types"
...
// Message represents a message sent to a ccntmract.
...
func IntrinsicGas(data []byte, ccntmractCreation, isHomestead bool, isEIP2028 bool) uint64 {
	// Set the starting gas for the raw transaction
	var gas uint64
	if ccntmractCreation && isHomestead {
		gas = params.TxGasCcntmractCreation
	} else {
		gas = params.TxGas
	}
	...
	if len(data) > 0 {
		...
		nonZeroGas := params.TxDataNonZeroGasFrcntmier
		if isEIP2028 {
...
func (st *StateTransition) to() common.Address {
	if st.msg == nil || st.msg.To() == nil /* ccntmract creation */ {
...
func (st *StateTransition) buyGas() (adjustedGas bool) {
	...
	if have, want := st.state.GetBalance(st.msg.From()), mgval; have.Cmp(want) < 0 {
		...
		if st.evm.ChainConfig().ChainID.Uint64() != constants.EIP155_CHAINID_MAINNET || st.evm.Ccntmext.BlockNumber.Uint64() >= 14100000 {
...
func (st *StateTransition) handleGasFee(adjustedGas bool) {
	if !adjustedGas || st.evm.Ccntmext.BlockNumber.Uint64() != RefundHeight {
		return
	}
...
func (st *StateTransition) TransitionDb() (*types.ExecutionResult, error) {
	...
	homestead := st.evm.ChainConfig().IsHomestead(st.evm.Ccntmext.BlockNumber)
	istanbul := st.evm.ChainConfig().IsIstanbul(st.evm.Ccntmext.BlockNumber)
	ccntmractCreation := msg.To() == nil
	...
	gas := IntrinsicGas(st.data, ccntmractCreation, homestead, istanbul)
	if st.gas < gas {
		vmerr = fmt.Errorf("%w: have %d, want %d", ErrIntrinsicGas, st.gas, gas)
		gas = st.gas
		// Check clause 6
	} else if msg.Value().Sign() > 0 && !st.evm.Ccntmext.CanTransfer(st.state, msg.From(), msg.Value()) {
		vmerr = fmt.Errorf("%w: address %v", ErrInsufficientFundsForTransfer, msg.From().Hex())
	}
	st.gas -= gas
	if vmerr == nil {
		if ccntmractCreation {
