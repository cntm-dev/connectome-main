...
const WitnessGlobalParamKey = "evm.witness" // value is the deployed hex addresss(evm form) of witness ccntmract.
...
func DecodeEventWitness(log *types.StorageLog) (*EventWitnessEvent, error) {
	if len(log.Topics) != 3 {
		return nil, errors.New("witness: wrcntm topic number")
	}
	if log.Topics[0] != EventWitnessedEventID {
		return nil, errors.New("witness: wrcntm event id")
	}
