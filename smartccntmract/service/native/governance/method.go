...
func registerCandidate(native *native.NativeService, flag string) error {
	...
	if native.Height >= config.GetSelfGovRegisterHeight() {
		...
		err = putPromisePos(native, ccntmract, promisePos)
		if err != nil {
		...
		indexBytes, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(PEER_INDEX), peerPubkeyPrefix))
		if err != nil {
		...
		if len(indexBytes) != 0 {
			...
			//get candidate index
			candidateIndex, err := getCandidateIndex(native, ccntmract)
			if err != nil {
			...
			err = putCandidateIndex(native, ccntmract, newCandidateIndex)
			if err != nil {
			...
			native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(PEER_INDEX), peerPubkeyPrefix), cstates.GenRawStorageItem(indexBytes))
		}
...
func executeSplit2(native *native.NativeService, ccntmract common.Address, view uint32) (uint64, error) {
	...
	currentPeerPoolMap, err := GetPeerPoolMap(native, ccntmract, view)
	if err != nil {
	...
	gasAddress, err := getGasAddress(native, ccntmract)
	if err != nil {
	...
	if gasAddress.Address == common.ADDRESS_EMPTY {
		...
		err := appCallTransferOng(native, utils.GovernanceCcntmractAddress, gasAddress.Address, dappIncome.Uint64())
		if err != nil {
	...
	for i := 0; i < int(config.K); i++ {
		...
		err = splitNodeFee(native, ccntmract, peersCandidate[i].PeerPubkey, peersCandidate[i].Address,
...
func executeAddressSplit(native *native.NativeService, ccntmract common.Address, authorizeInfo *AuthorizeInfo, preIfConsensus, ifConsensus bool, totalPos uint64, totalAmount uint64, peerAddress common.Address) (uint64, error) {
...
func splitNodeFee(native *native.NativeService, ccntmract common.Address, peerPubkey string, peerAddress common.Address, preIfConsensus, ifConsensus bool, totalPos uint64, nodeAmount uint64) error {
	peerPubkeyPrefix, err := hex.DecodeString(peerPubkey)
	if err != nil {
		return fmt.Errorf("hex.DecodeString, peerPubkey format error: %v", err)
	}
	...
	for has := iter.First(); has; has = iter.Next() {
		...
		//fee split
		splitAmount, err := executeAddressSplit(native, ccntmract, &authorizeInfo, preIfConsensus, ifConsensus, totalPos, amount, peerAddress)
		if err != nil {
