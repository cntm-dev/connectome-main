...
func addFileInfo(native *native.NativeService, fileInfo *FileInfo) {
	...
	fileInfoKey := GenFsFileInfoKey(ccntmract, fileInfo.FileOwner, fileInfo.FileHash)
...
func delFileInfo(native *native.NativeService, fileOwner common.Address, fileHash []byte) {
	ccntmract := native.CcntmextRef.CurrentCcntmext().CcntmractAddress
	fileInfoKey := GenFsFileInfoKey(ccntmract, fileOwner, fileHash)
...
func getFileHashList(native *native.NativeService, fileOwner common.Address) *FileHashList {
	...
	fileInfoPrefix := GenFsFileInfoPrefix(ccntmract, fileOwner)
