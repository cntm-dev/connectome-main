...
type Args struct {
	TargetAssetHash []byte // to ccntmract asset hash
	ToAddress       []byte
	Value           uint64
}
...
func (this *UnlockParam) Deserialization(source *common.ZeroCopySource) error {
	...
	if this.FromCcntmractHashBs, err = utils.DecodeVarBytes(source); err != nil {
		return fmt.Errorf("UnlockParam.Deserialization DecodeVarBytes FromCcntmractHashBs error:%s", err)
	}
