...
func PutBlockHeader(native *native.NativeService, blockHeader *ccom.Header, header []byte) error {
	ccntmract := utils.HeaderSyncCcntmractAddress
	...
	native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(BLOCK_HEADER), chainIDBytes, blockHash.ToArray()),
		cstates.GenRawStorageItem(header))
	native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(HEADER_INDEX), chainIDBytes, heightBytes),
		cstates.GenRawStorageItem(blockHash.ToArray()))
	native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(CURRENT_HEIGHT), chainIDBytes), cstates.GenRawStorageItem(heightBytes))
...
func GetHeaderByHeight(native *native.NativeService, chainID uint64, height uint32) (*ccom.Header, error) {
	...
	blockHashStore, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(HEADER_INDEX), chainIDBytes, heightBytes))
	if err != nil {
	...
	headerStore, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(BLOCK_HEADER), chainIDBytes, blockHashBytes))
	if err != nil {
...
func GetHeaderByHash(native *native.NativeService, chainID uint64, hash common.Uint256) (*ccom.Header, error) {
	...
	header := new(ccom.Header)
	headerStore, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(BLOCK_HEADER), chainIDBytes, hash.ToArray()))
	if err != nil {
...
func GetKeyHeights(native *native.NativeService, chainID uint64) (*KeyHeights, error) {
	...
	value, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(KEY_HEIGHTS), chainIDBytes))
	if err != nil {
...
func putKeyHeights(native *native.NativeService, chainID uint64, keyHeights *KeyHeights) error {
	...
	native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(KEY_HEIGHTS), chainIDBytes), cstates.GenRawStorageItem(sink.Bytes()))
	return nil
}
...
func getConsensusPeersByHeight(native *native.NativeService, chainID uint64, height uint32) (*ConsensusPeers, error) {
	...
	consensusPeerStore, err := native.CacheDB.Get(utils.ConcatKey(ccntmract, []byte(CONSENSUS_PEER), chainIDBytes, heightBytes))
	if err != nil {
...
func putConsensusPeers(native *native.NativeService, consensusPeers *ConsensusPeers) error {
	...
	native.CacheDB.Put(utils.ConcatKey(ccntmract, []byte(CONSENSUS_PEER), chainIDBytes, heightBytes), cstates.GenRawStorageItem(sink.Bytes()))
...
func notifyPutHeader(native *native.NativeService, chainID uint64, height uint32, blockHash string) {
	...
	native.Notifications = append(native.Notifications,
		&event.NotifyEventInfo{
			CcntmractAddress: utils.HeaderSyncCcntmractAddress,
