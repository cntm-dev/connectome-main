...
func InitHeaderSync() {
	native.Ccntmracts[utils.HeaderSyncCcntmractAddress] = RegisterHeaderSyncCcntmract
}

//Register methods of governance ccntmract
func RegisterHeaderSyncCcntmract(native *native.NativeService) {
...
func SyncGenesisHeader(native *native.NativeService) ([]byte, error) {
	...
	if err := params.Deserialization(common.NewZeroCopySource(native.Input)); err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("SyncGenesisHeader, ccntmract params deserialize error: %v", err)
	}
...
func SyncBlockHeader(native *native.NativeService) ([]byte, error) {
	...
	if err := params.Deserialization(common.NewZeroCopySource(native.Input)); err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("SyncBlockHeader, ccntmract params deserialize error: %v", err)
	}
