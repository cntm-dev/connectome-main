...
func LinkedlistInsert(native *native.NativeService, index []byte, item []byte, payload []byte) error {
	...
	if head == nil { //doubly-linked list ccntmains zero element
...
func linkedlistTest(native *native.NativeService) error {
	ccntmract1 := native.CcntmextRef.CurrentCcntmext().CcntmractAddress
	//ccntmract2 := []byte{ 0x01, 0x02 }

	{
		log.Trace("'empty linkedlist delete' test")
		//basic delete
		s, err := LinkedlistDelete(native, ccntmract1[:], []byte{byte(11)})
		if err != nil {
			return err
		}
		if s {
...

	{
		...
		//basic insert
		for i := 0; i < 10; i++ {
			err := LinkedlistInsert(native, ccntmract1[:], []byte{byte(i)}, []byte{byte(i * i)})
			if err != nil {
		...
		n, err := LinkedlistGetNumOfItems(native, ccntmract1[:])
		if err != nil {
			return err
		}
		if n != 10 {
...

	{
		...
		//insert & delete
		for i := 0; i < 10; i += 2 {
			suc, err := LinkedlistDelete(native, ccntmract1[:], []byte{byte(i)})
			if err != nil {
				//log.Errorf("")
				return err
			}
...

	{
		...
		for i := 2; i < 12; i++ {
			...
			err := LinkedlistInsert(native, ccntmract1[:], item, []byte("test"))
			if err != nil {
				return err
			}

			if i%2 == 0 {
				suc, err := LinkedlistDelete(native, ccntmract1[:], item)
				if err != nil {
					return err
				}
				if !suc {
