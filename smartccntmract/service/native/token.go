...
func Transfer(native *NativeService) (bool, error) {
	...
	for _, p := range transfers.Params {
		for _, s := range p.States {
			...
			toKey := append(p.Ccntmract.ToArray(), s.To.ToArray()...)
...
func getOntCcntmext() []byte {
	return genesis.OntCcntmractAddress.ToArray()
}

func getOngCcntmext() []byte {
	return genesis.OngCcntmractAddress.ToArray()
}

func getOntTotalSupplyKey() []byte {
	return append(getOntCcntmext(), totalSupplyName...)
}

func getOngTotalSupplyKey() []byte {
	return append(getOngCcntmext(), totalSupplyName...)
}
