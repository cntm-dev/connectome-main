...
func TestDeserializeGroup(t *testing.T) {
	...
	id1 := []byte("did:cntm:ASbxtSqrpmydpjqCUGDiQp2mzsfd4zFArs")
	id2 := []byte("did:cntm:AGxc3cdeB6QFvmZXzWhGwzuvohNtqaaaDw")
	g_ := &Group{
		Threshold: 1,
		Members: []interface{}{
			id0,
...
func groupCmp(a, b *Group) error {
	...
	for i := 0; i < len(a.Members); i++ {
		...
		case []byte:
			...
			if !ok {
				return fmt.Errorf("m%d: type error, cntm id expected", i)
			}
