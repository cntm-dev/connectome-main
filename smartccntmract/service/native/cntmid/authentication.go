...
func addNewAuthKey(srvc *native.NativeService) ([]byte, error) {
	...
	index, err := insertPk(srvc, encId, params.NewPublicKey.key, params.NewPublicKey.ccntmroller,
		false, true)
	if err != nil {
...
func addNewAuthKeyByRecovery(srvc *native.NativeService) ([]byte, error) {
	...
	// arg2: new public key ccntmroller
	...
	index, err := insertPk(srvc, encId, key, ccntmroller, false, true)
	if err != nil {
		return utils.BYTE_FALSE, errors.New("add auth key error, insertPk failed " + err.Error())
	}
...
func addNewAuthKeyByCcntmroller(srvc *native.NativeService) ([]byte, error) {
	source := common.NewZeroCopySource(srvc.Input)
	// arg0: id
	arg0, err := utils.DecodeVarBytes(source)
	if err != nil {
...
func setAuthKeyByCcntmroller(srvc *native.NativeService) ([]byte, error) {
	source := common.NewZeroCopySource(srvc.Input)
	// arg0: id
	arg0, err := utils.DecodeVarBytes(source)
	if err != nil {
	...
	if !isValid(srvc, encId) {
		return utils.BYTE_FALSE, errors.New("setAuthKeyByCcntmroller error: have not registered")
	}
...
func removeAuthKeyByCcntmroller(srvc *native.NativeService) ([]byte, error) {
	source := common.NewZeroCopySource(srvc.Input)
	// arg0: id
	arg0, err := utils.DecodeVarBytes(source)
	if err != nil {
	...
	if !isValid(srvc, encId) {
		return utils.BYTE_FALSE, errors.New("removeAuthKeyByCcntmroller error: have not registered")
	}
...
func getAuthentication(srvc *native.NativeService, encId []byte) ([]interface{}, error) {
	...
	for index, p := range publicKeys {
		if !p.revoked {
			cntmId, err := decodeID(encId)
			if err != nil {
				return nil, err
			}
			if p.isAuthentication && !p.isPkList {
				publicKey := new(publicKeyJson)
				publicKey.Id = fmt.Sprintf("%s#keys-%d", string(cntmId), index+1)
				publicKey.Ccntmroller = string(p.ccntmroller)
			...
			if p.isAuthentication && p.isPkList {
				authentication = append(authentication, fmt.Sprintf("%s#keys-%d", string(cntmId), index+1))
			}
		}
	}
