...
func regIdWithPublicKey(srvc *native.NativeService) ([]byte, error) {
	...
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: parsing argument 0 failed")
	} else if len(arg0) == 0 {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: invalid length of argument 0")
	}
	log.Debug("arg 0:", hex.EncodeToString(arg0), string(arg0))
	// arg1: public key
	arg1, err := utils.DecodeVarBytes(source)
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: parsing argument 1 failed")
	}

	log.Debug("arg 1:", hex.EncodeToString(arg1))

	if len(arg0) == 0 || len(arg1) == 0 {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: invalid argument")
	}

	if !account.VerifyID(string(arg0)) {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: invalid ID")
	}
	...
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: invalid public key")
	}
	addr := types.AddressFromPubKey(public)
	if !srvc.CcntmextRef.CheckWitness(addr) {
		return utils.BYTE_FALSE, errors.New("register cntm ID error: checking witness failed")
	}
...
func regIdWithAttributes(srvc *native.NativeService) ([]byte, error) {
	...
	if checkIDState(srvc, key) != flag_not_exist {
		return utils.BYTE_FALSE, errors.New("register cntm ID with attributes error: already registered")
	}
...
func addKey(srvc *native.NativeService) ([]byte, error) {
	...
	keyID, err := insertPk(srvc, key, arg1, ccntmroller, true, false)
	if err != nil {
...
func addKeyByIndex(srvc *native.NativeService) ([]byte, error) {
	log.Debug("ID ccntmract: AddKeyByIndex")
	source := common.NewZeroCopySource(srvc.Input)
	// arg0: id
	arg0, err := utils.DecodeVarBytes(source)
	if err != nil {
