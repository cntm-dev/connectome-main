...
type publicKeyJson struct {
	...
	Ccntmroller   string `json:"ccntmroller"`
...
func (this *publicKey) Serialization(sink *common.ZeroCopySink) {
	...
	sink.WriteVarBytes(this.ccntmroller)
...
func insertPk(srvc *native.NativeService, encId, pk, ccntmroller []byte, isPkList, isAuthentication bool) (uint32, error) {
	key := append(encId, FIELD_PK)
	if srvc.Height < config.GetNewOntIdHeight() {
		...
		size := len(publicKeys)
		publicKeys = append(publicKeys, &publicKey{pk, false, ccntmroller, isPkList, isAuthentication})
