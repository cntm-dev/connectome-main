...
func GetDDO(srvc *native.NativeService) ([]byte, error) {
	...
	// ccntmroller
	con, err := getCcntmroller(srvc, key)
	var2 := []byte{}
	if err == nil {
		switch t := con.(type) {
		case []byte:
...
func GetCcntmrollerJson(srvc *native.NativeService) ([]byte, error) {
	log.Debug("GetCcntmrollerJson")
	source := common.NewZeroCopySource(srvc.Input)
	// arg0: ID
	arg0, _, irregular, eof := source.NextVarBytes()
	if irregular || eof {
	...
	r, err := getCcntmrollerJson(srvc, encId)
	if err != nil {
		return nil, fmt.Errorf("getCcntmrollerJson failed: %s", err)
	}
...
func GetDocumentJson(srvc *native.NativeService) ([]byte, error) {
	...
	ccntmexts, err := getCcntmextsWithDefault(srvc, encId)
	if err != nil {
		return nil, fmt.Errorf("getCcntmextsWithDefault failed: %s", err)
	}
	id := string(arg0)
	...
	ccntmroller, err := getCcntmrollerJson(srvc, encId)
	if err != nil {
		return nil, fmt.Errorf("getCcntmroller failed: %s", err)
	}
