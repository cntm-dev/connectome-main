...
func RegisterOracleNode(native *NativeService) error {
	...
	oracleNodeBytes, err := native.CloneCache.Get(scommon.ST_STORAGE, concatKey(ccntmract, []byte(ORACLE_NODE), addressBytes))
	if err != nil {
	...
	native.CloneCache.Add(scommon.ST_STORAGE, concatKey(ccntmract, []byte(ORACLE_NODE), addressBytes), &cstates.StorageItem{Value: value})

	//cntm transfer
	err = appCallTransferOnt(native, address, genesis.OracleCcntmractAddress, new(big.Int).SetUint64(params.Guaranty))
	if err != nil {
		return errors.NewDetailErr(err, errors.ErrNoCode, "[appCallTransferOnt] Ont transfer error!")
	}
	//cntm transfer
	err = appCallTransferOng(native, address, genesis.FeeSplitCcntmractAddress, new(big.Int).SetInt64(ORACLE_NODE_FEE))
	if err != nil {
...
func QuitOracleNode(native *NativeService) error {
	...
	//cntm transfer
	err = appCallTransferOnt(native, address, genesis.OracleCcntmractAddress, new(big.Int).SetUint64(oracleNode.Guaranty))
	if err != nil {
		return errors.NewDetailErr(err, errors.ErrNoCode, "[appCallTransferOnt] Ont transfer error!")
	}

	native.CloneCache.Delete(scommon.ST_STORAGE, concatKey(ccntmract, []byte(ORACLE_NODE), addressBytes))
...
func CreateOracleRequest(native *NativeService) error {
	...
	addCommonEvent(native, ccntmract, CREATE_ORACLE_REQUEST, params.Request)
