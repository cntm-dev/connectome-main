...
type TokenTransfer struct {
	Ccntmract common.Uint160
	States   []*State
}

func (this *TokenTransfer) Serialize(w io.Writer) error {
	if _, err := this.Ccntmract.Serialize(w); err != nil {
		return errors.NewDetailErr(err, errors.ErrNoCode, "[TokenTransfer Serialize] Ccntmract error!")
	}
...
func (this *TokenTransfer) Deserialize(r io.Reader) error {
	ccntmract := new(common.Uint160)
	if err := ccntmract.Deserialize(r); err != nil {
		return errors.NewDetailErr(err, errors.ErrNoCode, "[TokenTransfer Deserialize] Ccntmract error!")
	}
	this.Ccntmract = *ccntmract
