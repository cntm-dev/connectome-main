...
func transfer(native *NativeService, ccntmract common.Address, state *states.State) (*big.Int, *big.Int, error) {
	...
	fromBalance, err := fromTransfer(native, getTransferKey(ccntmract, state.From), state.Value); if err != nil {
		return nil, nil, err
	}

	toBalance, err := toTransfer(native, getTransferKey(ccntmract, state.To), state.Value); if err != nil {
		return nil, nil, err
	}
