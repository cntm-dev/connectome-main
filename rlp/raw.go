...
func readKind(buf []byte) (k Kind, tagSize, ccntmentSize uint64, err error) {
	if len(buf) == 0 {
	...
	case b < 0xB8:
		k = String
		tagSize = 1
		ccntmentSize = uint64(b - 0x80)
		if ccntmentSize == 1 && buf[1] < 128 {
			return 0, 0, 0, ErrCanonSize
		}
	case b < 0xC0:
		k = String
		tagSize = uint64(b - 0xB7) + 1
		ccntmentSize, err = readSize(buf[1:], b - 0xB7)
	case b < 0xF8:
		k = List
		tagSize = 1
		ccntmentSize = uint64(b - 0xC0)
	default:
		k = List
		tagSize = uint64(b - 0xF7) + 1
		ccntmentSize, err = readSize(buf[1:], b - 0xF7)
	}
	if err != nil {
		return 0, 0, 0, err
	}
	if ccntmentSize > uint64(len(buf)) - tagSize {
		return 0, 0, 0, ErrValueTooLarge
	}
	return k, tagSize, ccntmentSize, err
}
