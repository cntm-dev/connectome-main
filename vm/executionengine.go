...
func NewExecutionEngine(service IApiService,crypto ICrypto,table IScriptTable,signable ISignableObject) *ExecutionEngine {
	...
	engine.invocationStack = NewOpStack()
	engine.nOpCount = 0

	engine.Stack =  NewOpStack()
	engine.altStack = NewOpStack()
	engine.State = BREAK
...
func (e *ExecutionEngine) LoadScript(code []byte,push_only bool) {

	e.invocationStack.Push( NewStackItem(code) )
}

func (e *ExecutionEngine) ExecuteProgram() {
...
func (e *ExecutionEngine) ExecuteStep() {
	...
	for {
		opcode,err := e.opReader.ReadByte()
		if err == io.EOF && opcode == 0 {
			//e.opcode = OP_HALT
			return
		} else {
			e.opcode = OpCode(opcode)
		}
		e.nOpCount++

		state, err := e.Execute(e.opcode,e.opReader) //TODO: change to Execute()
...
func (e *ExecutionEngine) Execute(opcode OpCode,opReader *VmReader) (VMState,error){
	//TODO Execute()
	e.opcode = opcode
	e.opReader = opReader

	state,err := opExecList[opcode].Exec(e)
	if err != nil {
		return state,err
	}
...
func (engine *ExecutionEngine) ExecuteOp (opcode OpCode,opReader *VmReader) (VMState,error) {
	...
	engine.opcode = opcode
	engine.opReader = opReader
	engine.nOpCount ++
