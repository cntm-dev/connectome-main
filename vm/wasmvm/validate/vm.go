...
// a block reprsents an instruction sequence preceeded by a ccntmrol flow operator
// it is used to verify that the block signature set by the operator is the correct
// one when the block ends
type block struct {
	pc          int            // the pc where the ccntmrol flow operator starting the block is located
	stackTop    int            // stack top when the block started
	blockType   wasm.BlockType // block_type signature of the ccntmrol operator
