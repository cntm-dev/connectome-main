...
func (s *vmstack) push(vm *VM) error {
	if s.top+1 == len(s.stack) {
		return errors.NewErr(fmt.Sprintf("[vm stack push] stack is full, only support %d ccntmracts calls", VM_STACK_DEPTH))
	}
	s.stack[s.top+1] = vm
	s.top += 1
	return nil
...
func (e *ExecutionEngine) InitCall(caller common.Address,
	...
	return e.call(caller, code, input, CcntmRACT_INIT_METHOD, ver)
...
func (e *ExecutionEngine) call(caller common.Address,
	...
	if ver > 0 { //production ccntmract version
		...
		vm.VMCode = code
		vm.CcntmractAddress = common.AddressFromVmCode(code)
