...
// This is in ccntmrast with original WebAssembly bytecode, where the target
...
// A ccntmrol operator sequence isn't allowed to access nor modify (pop) operands
...
// block's signature.
// Instead of creating a new stack every time we enter a ccntmrol structure,
// we record the current stack height on encountering a ccntmrol operator.
...
// Unlike other ccntmrol instructions, br_table does jumps and discarding all
// by itself.
type Target struct {
...
// block stores the information relevant for a block created by a ccntmrol operator
// sequence (if...else...end, loop...end, and block...end)
type block struct {
	// the byte offset to which the ccntminuation of the label
...
func Compile(disassembly []disasm.Instr) ([]byte, []*BranchTable) {
	...
	for _, instr := range disassembly {
		...
		case ops.Else:
			...
			// this is no lcntmer an if block
