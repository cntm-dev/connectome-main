...
type EnvCall struct {
	...
	envReturns bool
	envPreCtx  ccntmext
	Message    []interface{} //the 'Message' field is for the EOS ccntmract like parameters
}
...
// VM is the execution ccntmext for executing WebAssembly bytecode.
type VM struct {
	ctx ccntmext
...
// the VM's module.
//insideCall :true (call ccntmract)
...
//start a new vm
func (vm *VM) CallProductCcntmract(caller common.Address,codeHash common.Address,module *wasm.Module, actionName []byte, arg []byte) (uint64, error) {
...
func (vm *VM) loadModule(module *wasm.Module) error {
	...
	if module.Data != nil {
		...
		for _, entry := range module.Data.Entries {
			...
			if bytes.Ccntmains(entry.Data, []byte{byte(0)}) {
