...
const (
	// Constants
	OP_0   OpCode  = 0x00 // An empty array of bytes is pushed cntmo the stack. (This is not a no-op: an item is added to the stack.)
	OP_FALSE   OpCode   = OP_0
	OP_PUSHBYTES1  OpCode  = 0x01 // 0x01-0x4B The next  OpCode  bytes is data to be pushed cntmo the stack
	OP_PUSHBYTES33  OpCode  = 0x21
	OP_PUSHBYTES64  OpCode  = 0x40
	OP_PUSHBYTES75  OpCode  = 0x4B
	OP_PUSHDATA1  OpCode  = 0x4C // The next byte ccntmains the number of bytes to be pushed cntmo the stack.
	OP_PUSHDATA2  OpCode  = 0x4D // The next two bytes ccntmain the number of bytes to be pushed cntmo the stack.
	OP_PUSHDATA4  OpCode  = 0x4E // The next four bytes ccntmain the number of bytes to be pushed cntmo the stack.
	OP_1NEGATE  OpCode  = 0x4F // The number -1 is pushed cntmo the stack.
	//OP_RESERVED  OpCode  = 0x50 // Transaction is invalid unless occuring in an unexecuted OP_IF branch
	OP_1  OpCode  = 0x51 // The number 1 is pushed cntmo the stack.
	OP_TRUE  OpCode  = OP_1
	OP_2  OpCode  = 0x52 // The number 2 is pushed cntmo the stack.
	OP_3  OpCode  = 0x53 // The number 3 is pushed cntmo the stack.
	OP_4  OpCode  = 0x54 // The number 4 is pushed cntmo the stack.
	OP_5  OpCode  = 0x55 // The number 5 is pushed cntmo the stack.
	OP_6  OpCode  = 0x56 // The number 6 is pushed cntmo the stack.
	OP_7  OpCode  = 0x57 // The number 7 is pushed cntmo the stack.
	OP_8  OpCode  = 0x58 // The number 8 is pushed cntmo the stack.
	OP_9  OpCode  = 0x59 // The number 9 is pushed cntmo the stack.
	OP_10  OpCode  = 0x5A // The number 10 is pushed cntmo the stack.
	OP_11  OpCode  = 0x5B // The number 11 is pushed cntmo the stack.
	OP_12  OpCode  = 0x5C // The number 12 is pushed cntmo the stack.
	OP_13  OpCode  = 0x5D // The number 13 is pushed cntmo the stack.
	OP_14  OpCode  = 0x5E // The number 14 is pushed cntmo the stack.
	OP_15  OpCode  = 0x5F // The number 15 is pushed cntmo the stack.
	OP_16  OpCode  = 0x60 // The number 16 is pushed cntmo the stack.
	...
	// Stack
	OP_TOALTSTACK  OpCode  = 0x6B // Puts the input cntmo the top of the alt stack. Removes it from the main stack.
	OP_FROMALTSTACK  OpCode  = 0x6C // Puts the input cntmo the top of the main stack. Removes it from the alt stack.
	...
	OP_2OVER  OpCode  = 0x70 // Copies the pair of items two spaces back in the stack to the frcntm.
	...
	OP_DEPTH  OpCode  = 0x74 // Puts the number of stack items cntmo the stack.
