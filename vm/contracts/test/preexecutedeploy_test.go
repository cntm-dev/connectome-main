...
func TestPreExecuteContractWasmDeploy(t *testing.T) {
	...
	/** file: test_create.wat
		(module
		  ...
		  (import "env" "conntectome_return" (func (;0;) (type 1)))
		  (func (;1;) (type 0)
			i32.const 0
		    i64.const 2222
			i64.store
	...
	mutable, _ := utils.NewDeployCodeTransaction(0, 100000000, code, payload.CNTMVM_TYPE, "name", "version",
		"author", "email", "desc")
	_ = utils.SignTransaction(acct, mutable)
	tx, err := mutable.IntoImmutable()
	_, err = testLedgerStore.PreExecuteContract(tx)
	assert.EqualError(t, err, "this code is wasm binary. can not deployed as cntm contract")
