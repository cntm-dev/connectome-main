...
func RuntimeGetTime(service *CntmVmService, engine *vm.Executor) error {
...
func RuntimeCheckWitness(service *CntmVmService, engine *vm.Executor) error {
...
func RuntimeSerialize(service *CntmVmService, engine *vm.Executor) error {
...
func RuntimeDeserialize(service *CntmVmService, engine *vm.Executor) error {
	data, err := engine.EvalStack.PopAsBytes()
	if err != nil {
...
func RuntimeVerifyMutiSig(service *CntmVmService, engine *vm.Executor) error {
	data, err := engine.EvalStack.PopAsBytes()
	if err != nil {
		return err
	}
...
func RuntimeNotify(service *CntmVmService, engine *vm.Executor) error {
	...
	states, err := item.ConvertCntmVmValueHexString()
	if err != nil {
		return err
	}
...
func RuntimeLog(service *CntmVmService, engine *vm.Executor) error {
	...
	scv := sitem.Dump()
	log.Debugf("[CntmContract]Debug:%s\n", scv)
	return nil
}

func RuntimeGetTrigger(service *CntmVmService, engine *vm.Executor) error {
	return engine.EvalStack.PushInt64(int64(0))
}

func RuntimeBase58ToAddress(service *CntmVmService, engine *vm.Executor) error {
	item, err := engine.EvalStack.PopAsBytes()
	if err != nil {
		return err
	}
...
func RuntimeAddressToBase58(service *CntmVmService, engine *vm.Executor) error {
	item, err := engine.EvalStack.PopAsBytes()
	if err != nil {
		return err
	}
...
func RuntimeGetCurrentBlockHash(service *CntmVmService, engine *vm.Executor) error {
