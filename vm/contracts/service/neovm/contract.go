...
func ContractCreate(service *CntmVmService, engine *vm.Executor) error {
...
func ContractMigrate(service *CntmVmService, engine *vm.Executor) error {
	contract, err := isContractParamValid(engine)
	if err != nil {
...
func ContractDestory(service *CntmVmService, engine *vm.Executor) error {
	context := service.ContextRef.CurrentContext()
	if context == nil {
...
func ContractGetStorageContext(service *CntmVmService, engine *vm.Executor) error {
...
func ContractGetCode(service *CntmVmService, engine *vm.Executor) error {
...
func isContractParamValid(engine *vm.Executor) (*payload.DeployCode, error) {
	...
	if contract.VmType() != payload.CNTMVM_TYPE {
		return nil, fmt.Errorf("[Contract] expect CNTMVM_TYPE. get WASMVM_TYPE")
	}

	return contract, nil
}

func isContractExist(service *CntmVmService, contractAddress common.Address) error {
