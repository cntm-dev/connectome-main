...
func BlockChainGetHeight(service *CntmVmService, engine *vm.Executor) error {
...
func BlockChainGetHeader(service *CntmVmService, engine *vm.Executor) error {
	var (
		header *types.Header
		err    error
	)
	...
	l := len(data)
	if l <= 5 {
		b := common.BigIntFromCntmBytes(data)
...
func BlockChainGetBlock(service *CntmVmService, engine *vm.Executor) error {
	data, err := engine.EvalStack.PopAsBytes()
	if err != nil {
		return err
	}
...
func BlockChainGetTransaction(service *CntmVmService, engine *vm.Executor) error {
...
func BlockChainGetContract(service *CntmVmService, engine *vm.Executor) error {
...
func BlockChainGetTransactionHeight(service *CntmVmService, engine *vm.Executor) error {
	d, err := engine.EvalStack.PopAsBytes()
	if err != nil {
		return err
	}
