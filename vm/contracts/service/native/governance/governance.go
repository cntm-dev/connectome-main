...
//Governance contract:
//Users can apply for a candidate node to join consensus selection, deposit CNTM to authorize for candidate nodes, quit selection and unAuthorize for candidate nodes through this contract.
//CNTM deposited in the contract can get CNTG bonus which come from transaction fee of the network.
package governance
...
// candidate fee must >= 1 CNTG
var MIN_CANDIDATE_FEE = uint64(math.Pow(10, constants.CNTG_DECIMALS))
...
func InitGovernance() {
	native.Ccntmracts[utils.GovernanceCcntmractAddress] = RegisterGovernanceCcntmract
}

//Register methods of governance contract
func RegisterGovernanceCcntmract(native *native.NativeService) {
	...
	native.Register(WITHDRAW_CNTG, WithdrawCntg)
...
//Init governance contract, include Cbft config, global param and cntmid admin.
func InitConfig(native *native.NativeService) ([]byte, error) {
	configuration := new(config.CbftConfig)
	...
	err = CheckCbftConfig(configuration)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("checkCbftConfig failed: %v", err)
	}
	...
	//init admin cntmid
	err = appCallInitCcntmractAdmin(native, []byte(configuration.Admincntmid))
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallInitCcntmractAdmin error: %v", err)
	}
...
//Users can register a candidate node with a authorized cntmid.
//Candidate node can be authorized and become consensus node according to their pos.
//Candidate node can get cntg bonus according to their pos.
...
//Ccntmracts can register a candidate node with a authorized cntmid after approving cntm to governance contract before invoke this function.
//Candidate node can be authorized and become consensus node according to their pos.
//Candidate node can get cntg bonus according to their pos.
...
//Unregister a registered candidate node, will remove node from pool, and unfreeze deposit cntm.
...
//Only approved candidate node can participate in consensus selection and get cntg bonus.
...
//Reject a registered candidate node, remove node from pool and unfreeze deposit cntm
//Only approved candidate node can participate in consensus selection and get cntg bonus.
...
//Authorize for a node by depositing CNTM in this governance contract, used by users
...
//Authorize for a node by depositing CNTM in this governance contract, used by contracts
...
//UnAuthorize for a node by redeeming CNTM from this governance contract
...
//Withdraw unfreezed CNTM deposited in this governance contract.
func Withdraw(native *native.NativeService) ([]byte, error) {
	...
	//cntm transfer
	err = appCallTransferCntm(native, utils.GovernanceCcntmractAddress, address, total)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
	}
...
//Transfer all punished CNTM of a black node to a certain address
...
//Withdraw unbounded CNTG according to deposit CNTM in this governance contract
func WithdrawCntg(native *native.NativeService) ([]byte, error) {
	params := new(WithdrawCntgParam)
	if err := params.Deserialization(common.NewZeroCopySource(native.Input)); err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("deserialize, deserialize transferPenaltyParam error: %v", err)
	}
	contract := native.CcntmextRef.CurrentCcntmext().CcntmractAddress
	...
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("withdrawCntg, checkWitness error: %v", err)
	}

	// cntm transfer to trigger unboundcntg
	err = appCallTransferCntm(native, utils.GovernanceCcntmractAddress, utils.GovernanceCcntmractAddress, 1)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
	}
	...
	amount := utils.CalcUnbindCntg(totalStake.Stake, preTimeOffset, timeOffset)
	err = appCallTransferFromCntg(native, utils.GovernanceCcntmractAddress, utils.CntmCcntmractAddress, totalStake.Address, amount)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallTransferFromCntg, transfer from cntg error: %v", err)
	}
...
//Change the status if node can receive authorization from cntm holders
...
func WithdrawFee(native *native.NativeService) ([]byte, error) {
	...
	//cntg transfer
	err = appCallTransferCntg(native, utils.GovernanceCcntmractAddress, params.Address, fee)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallTransferCntg, cntg transfer error: %v", err)
	}
...
func AddInitPos(native *native.NativeService) ([]byte, error) {
	...
	//cntm transfer
	err = appCallTransferCntm(native, params.Address, utils.GovernanceCcntmractAddress, uint64(params.Pos))
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
	}
