...
type PenaltyStake struct { //table record penalty stake of peer
	...
	TimeOffset   uint32 //time used for calculate unbound cntg
	Amount       uint64 //unbound cntg that this penalty unbounded
}
...
type PeerAttributes struct {
	PeerPubkey   string
	MaxAuthorize uint64 //max authorzie pos this peer can receive(number of cntm), set by peer owner
...
type SplitFeeAddress struct { //table record each address's cntg motivation
	Address common.Address
	Amount  uint64
}
