...
type GlobalParam struct {
	CandidateFee uint64 //unit: 10^-9 cntg
...
type GlobalParam2 struct {
	MinAuthorizePos      uint32 //min CNTM of each authorization, 500 default
...
type WithdrawCntgParam struct {
	Address common.Address
}

func (this *WithdrawCntgParam) Serialization(sink *common.ZeroCopySink) {
	sink.WriteVarBytes(this.Address[:])
}

func (this *WithdrawCntgParam) Deserialization(source *common.ZeroCopySource) error {
	address, err := utils.DecodeAddress(source)
	if err != nil {
		return fmt.Errorf("utils.ReadAddress, deserialize address error: %v", err)
	}
