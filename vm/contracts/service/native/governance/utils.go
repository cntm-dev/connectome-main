...
import (
	...
	"github.com/conntectome/cntm-crypto/vrf"
	"github.com/conntectome/cntm/common"
	"github.com/conntectome/cntm/common/config"
	"github.com/conntectome/cntm/common/serialization"
	Cbftconfig "github.com/conntectome/cntm/consensus/Cbft/config"
...
func appCallTransferCntm(native *native.NativeService, from common.Address, to common.Address, amount uint64) error {
	err := appCallTransfer(native, utils.CntmCcntmractAddress, from, to, amount)
	if err != nil {
		return fmt.Errorf("appCallTransferCntm, appCallTransfer error: %v", err)
	}
	return nil
}

func appCallTransferCntg(native *native.NativeService, from common.Address, to common.Address, amount uint64) error {
	err := appCallTransfer(native, utils.CntgCcntmractAddress, from, to, amount)
	if err != nil {
		return fmt.Errorf("appCallTransferCntg, appCallTransfer error: %v", err)
	}
	return nil
}
...
func appCallTransferFromCntm(native *native.NativeService, sender common.Address, from common.Address, to common.Address, amount uint64) error {
	err := appCallTransferFrom(native, utils.CntmCcntmractAddress, sender, from, to, amount)
	if err != nil {
		return fmt.Errorf("appCallTransferFromCntm, appCallTransferFrom error: %v", err)
	}
	return nil
}

func appCallTransferFromCntg(native *native.NativeService, sender common.Address, from common.Address, to common.Address, amount uint64) error {
	err := appCallTransferFrom(native, utils.CntgCcntmractAddress, sender, from, to, amount)
	if err != nil {
		return fmt.Errorf("appCallTransferFromCntg, appCallTransferFrom error: %v", err)
	}
	return nil
}
...
func getCntgBalance(native *native.NativeService, address common.Address) (uint64, error) {
	sink := common.ZeroCopySink{}
	utils.EncodeAddress(&sink, address)

	value, err := native.NativeCall(utils.CntgCcntmractAddress, "balanceOf", sink.Bytes())
	if err != nil {
		return 0, fmt.Errorf("getCntgBalance, appCall error: %v", err)
	}
	balance := common.BigIntFromCntmBytes(value).Uint64()
	return balance, nil
}
...
func validatePeerPubKeyFormat(pubkey string) error {
	pk, err := Cbftconfig.Pubkey(pubkey)
	if err != nil {
...
func CheckCbftConfig(configuration *config.CbftConfig) error {
...
func getConfig(native *native.NativeService, contract common.Address) (*Configuration, error) {
	...
	configBytes, err := native.CacheDB.Get(utils.ConcatKey(contract, []byte(Cbft_CONFIG)))
	if err != nil {
...
func putConfig(native *native.NativeService, contract common.Address, config *Configuration) error {
	native.CacheDB.Put(utils.ConcatKey(contract, []byte(Cbft_CONFIG)), cstates.GenRawStorageItem(common.SerializeToBytes(config)))
	return nil
}
...
func appCallInitCcntmractAdmin(native *native.NativeService, admincntmid []byte) error {
	params := &auth.InitCcntmractAdminParam{
		Admincntmid: admincntmid,
	}
	if _, err := native.NativeCall(utils.AuthCcntmractAddress, "initCcntmractAdmin", common.SerializeToBytes(params)); err != nil {
		return fmt.Errorf("appCallInitCcntmractAdmin, appCall error: %v", err)
	}
	return nil
}
...
func appCallVerifyToken(native *native.NativeService, contract common.Address, caller []byte, fn string, keyNo uint64) error {
	params := &auth.VerifyTokenParam{
		CcntmractAddr: contract,
		Caller:       caller,
		Fn:           fn,
		KeyNo:        keyNo,
	}
	ok, err := native.NativeCall(utils.AuthCcntmractAddress, "verifyToken", common.SerializeToBytes(params))
	if err != nil {
