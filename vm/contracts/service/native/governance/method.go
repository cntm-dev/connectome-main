...
func registerCandidate(native *native.NativeService, flag string) error {
	...
	case "transfer":
		//cntm transfer
		err = appCallTransferCntm(native, params.Address, utils.GovernanceCcntmractAddress, uint64(params.InitPos))
		if err != nil {
			return fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
		}

		//cntg transfer
		err = appCallTransferCntg(native, params.Address, utils.GovernanceCcntmractAddress, globalParam.CandidateFee)
		if err != nil {
			return fmt.Errorf("appCallTransferCntg, cntg transfer error: %v", err)
		}
	case "transferFrom":
		//cntm transfer from
		err = appCallTransferFromCntm(native, utils.GovernanceCcntmractAddress, params.Address, utils.GovernanceCcntmractAddress, uint64(params.InitPos))
		if err != nil {
			return fmt.Errorf("appCallTransferFromCntm, cntm transfer error: %v", err)
		}

		//cntg transfer from
		err = appCallTransferFromCntg(native, utils.GovernanceCcntmractAddress, params.Address, utils.GovernanceCcntmractAddress, globalParam.CandidateFee)
		if err != nil {
			return fmt.Errorf("appCallTransferFromCntg, cntg transfer error: %v", err)
		}
	}
...
func authorizeForPeer(native *native.NativeService, flag string) error {
	...
	case "transfer":
		//cntm transfer
		err = appCallTransferCntm(native, params.Address, utils.GovernanceCcntmractAddress, total)
		if err != nil {
			return fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
		}
	case "transferFrom":
		//cntm transfer from
		err = appCallTransferFromCntm(native, utils.GovernanceCcntmractAddress, params.Address, utils.GovernanceCcntmractAddress, total)
		if err != nil {
			return fmt.Errorf("appCallTransferFromCntm, cntm transfer error: %v", err)
		}
	}
...
func blackQuit(native *native.NativeService, contract common.Address, peerPoolItem *PeerPoolItem) error {
	...
	err := appCallTransferCntm(native, utils.GovernanceCcntmractAddress, utils.GovernanceCcntmractAddress, peerPoolItem.InitPos)
	if err != nil {
		return fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
	}
...
func depositTotalStake(native *native.NativeService, contract common.Address, address common.Address, stake uint64) error {
	...
	amount := utils.CalcUnbindCntg(preStake, preTimeOffset, timeOffset)
	err = appCallTransferFromCntg(native, utils.GovernanceCcntmractAddress, utils.CntmCcntmractAddress, totalStake.Address, amount)
	if err != nil {
		return fmt.Errorf("appCallTransferFromCntg, transfer from cntg error: %v", err)
	}
...
func withdrawPenaltyStake(native *native.NativeService, contract common.Address, peerPubkey string, address common.Address) error {
	...
	//cntm transfer
	err = appCallTransferCntm(native, utils.GovernanceCcntmractAddress, address, preStake)
	if err != nil {
		return fmt.Errorf("appCallTransferCntm, cntm transfer error: %v", err)
	}
	//cntg approve
	err = appCallTransferFromCntg(native, utils.GovernanceCcntmractAddress, utils.CntmCcntmractAddress, address, amount+preAmount)
	if err != nil {
		return fmt.Errorf("appCallTransferFromCntg, transfer from cntg error: %v", err)
	}
...
func executeSplit(native *native.NativeService, contract common.Address, view uint32) error {
	...
	balance, err := getCntgBalance(native, utils.GovernanceCcntmractAddress)
	if err != nil {
		return fmt.Errorf("executeSplit, getCntgBalance error: %v", err)
	}
	//get globalParam
	globalParam, err := getGlobalParam(native, contract)
	if err != nil {
	...
	for i := 0; i < int(config.K); i++ {
		...
		err = appCallTransferCntg(native, utils.GovernanceCcntmractAddress, address, nodeAmount)
		if err != nil {
			return fmt.Errorf("executeSplit, cntg transfer error: %v", err)
		}
	}
...
func executeSplit2(native *native.NativeService, contract common.Address, view uint32) (uint64, error) {
	...
	balance, err := getCntgBalance(native, utils.GovernanceCcntmractAddress)
	if err != nil {
		return splitSum, fmt.Errorf("executeSplit, getCntgBalance error: %v", err)
	}
	...
	if gasAddress.Address == common.ADDRESS_EMPTY {
		...
		err := appCallTransferCntg(native, utils.GovernanceCcntmractAddress, gasAddress.Address, dappIncome.Uint64())
		if err != nil {
			return splitSum, fmt.Errorf("appCallTransferCntg, appCallTransferCntg error: %v", err)
		}
	}
