 ...
 */
func initContractAdmin(native *native.NativeService, contractAddr common.Address, cntmid []byte) (bool, error) {
	...
	err = putContractAdmin(native, contractAddr, cntmid)
	if err != nil {
		return false, err
	}
	return true, nil
...
func InitContractAdmin(native *native.NativeService) ([]byte, error) {
	...
	if !account.VerifyID(string(param.Admincntmid)) {
		return nil, fmt.Errorf("[initContractAdmin] invalid param: admincntmid is %x", param.Admincntmid)
	}
	ret, err := initContractAdmin(native, invokeAddr, param.Admincntmid)
	if err != nil {
	...
	msg := []interface{}{"initContractAdmin", invokeAddr.ToHexString(), string(param.Admincntmid)}
	pushEvent(native, msg)
	return utils.BYTE_TRUE, nil
}

func transfer(native *native.NativeService, contractAddr common.Address, newAdmincntmid []byte, keyNo uint64) (bool, error) {
	admin, err := getContractAdmin(native, contractAddr)
	if err != nil {
	...
	utils.PutBytes(native, adminKey, newAdmincntmid)
	return true, nil
}
...
func Transfer(native *native.NativeService) ([]byte, error) {
	...
	if !account.VerifyID(string(param.NewAdmincntmid)) {
		return nil, fmt.Errorf("[transfer] invalid param: newAdmincntmid is %x", param.NewAdmincntmid)
	}
	//prepare event msg
	...
	//call transfer func
	ret, err := transfer(native, param.ContractAddr, param.NewAdmincntmid, param.KeyNo)
	if err != nil {
...
func AssignFuncsToRole(native *native.NativeService) ([]byte, error) {
	...
	if bytes.Compare(admin, param.Admincntmid) != 0 {
		log.Debugf("[assignFuncsToRole] invalid param: admincntmid doesn't match %s != %s",
			string(param.Admincntmid), string(admin))
		pushEvent(native, failState)
		return utils.BYTE_FALSE, nil
	}
	ret, err := verifySig(native, param.Admincntmid, param.KeyNo)
	if err != nil {
		return nil, fmt.Errorf("[assignFuncsToRole] verify admin's signature failed: %v", err)
	}
	if !ret {
		log.Debugf("[assignFuncsToRole] verifySig return false: admincntmid=%s, keyNo=%d",
...
func assignToRole(native *native.NativeService, param *cntmidsToRoleParam) (bool, error) {
	...
	if bytes.Compare(admin, param.Admincntmid) != 0 {
		log.Debugf("param's admincntmid doesn't match: %s != %s", string(param.Admincntmid),
			string(admin))
		return false, nil
	}
	valid, err := verifySig(native, param.Admincntmid, param.KeyNo)
	if err != nil {
		return false, fmt.Errorf("verify admin's signature failed: %v", err)
	}
	if !valid {
		log.Debugf("[assigncntmidsToRole] verifySig return false: admincntmid=%s, keyNo=%d",
			string(admin), param.KeyNo)
		return false, nil
	}
	...
	for _, p := range param.Persons {
		...
		tokens, err := getcntmidToken(native, param.ContractAddr, p)
		if err != nil {
			return false, fmt.Errorf("getcntmidToken failed: %v", err)
		}
		if tokens == nil {
		...
		err = putcntmidToken(native, param.ContractAddr, p, tokens)
		if err != nil {
			return false, err
		}
	}
...
func AssigncntmidsToRole(native *native.NativeService) ([]byte, error) {
	//deserialize param
	param := new(cntmidsToRoleParam)
	source := common.NewZeroCopySource(native.Input)
	if err := param.Deserialization(source); err != nil {
		return nil, fmt.Errorf("[assigncntmidsToRole] deserialize param failed: %v", err)
	}

	if param.Role == nil {
		return nil, fmt.Errorf("[assigncntmidsToRole] invalid param: role is nil")
	}
	for i, cntmid := range param.Persons {
		if !account.VerifyID(string(cntmid)) {
			return nil, fmt.Errorf("[assigncntmidsToRole] invalid param: param.Persons[%d]=%s",
				i, string(cntmid))
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[assigncntmidsToRole] failed: %v", err)
	}

	contract := param.ContractAddr.ToHexString()
	failState := []interface{}{"assigncntmidsToRole", contract, false}
	sucState := []interface{}{"assigncntmidsToRole", contract, true}
	if ret {
		pushEvent(native, sucState)
		return utils.BYTE_TRUE, nil
	} else {
...
func getAuthToken(native *native.NativeService, contractAddr common.Address, cntmid, role []byte) (*AuthToken, error) {
	tokens, err := getcntmidToken(native, contractAddr, cntmid)
	if err != nil {
	...
	status, err := getDelegateStatus(native, contractAddr, cntmid)
	if err != nil {
...
func hasRole(native *native.NativeService, contractAddr common.Address, cntmid, role []byte) (bool, error) {
	token, err := getAuthToken(native, contractAddr, cntmid, role)
	if err != nil {
		return false, err
	}
	if token == nil {
...
func getLevel(native *native.NativeService, contractAddr common.Address, cntmid, role []byte) (uint8, error) {
	token, err := getAuthToken(native, contractAddr, cntmid, role)
	if err != nil {
		return 0, err
	}
...
func delegate(native *native.NativeService, contractAddr common.Address, from []byte, to []byte,
	...
	if !account.VerifyID(string(to)) {
		return false, fmt.Errorf("can not pass cntmid validity test: to=%s", string(to))
	}
...
func verifyToken(native *native.NativeService, contractAddr common.Address, caller []byte, fn string, keyNo uint64) (bool, error) {
	...
	tokens, err := getcntmidToken(native, contractAddr, caller)
	if err != nil {
		return false, fmt.Errorf("getcntmidToken failed: %v", err)
	}
	if tokens != nil {
...
func verifySig(native *native.NativeService, cntmid []byte, keyNo uint64) (bool, error) {
	...
	args := sink.Bytes()
	ret, err := native.NativeCall(utils.cntmidContractAddress, "verifySignature", args)
	if err != nil {
		return false, err
	}
...
func RegisterAuthContract(native *native.NativeService) {
	...
	native.Register("assigncntmidsToRole", AssigncntmidsToRole)
