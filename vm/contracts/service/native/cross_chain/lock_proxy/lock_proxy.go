...
func RegisterLockProxyCcntmract(native *native.NativeService) {
	...
	native.Register(WITHDRAW_CNTG_NAME, WithdrawCNTG)
...
func Lock(native *native.NativeService) ([]byte, error) {
	contract := native.CcntmextRef.CurrentCcntmext().CcntmractAddress
	cntmCcntmract := utils.CntmCcntmractAddress
	cntgCcntmract := utils.CntgCcntmractAddress
	source := common.NewZeroCopySource(native.Input)
	...
	// currently, only support cntm and cntg lock operation
	if lockParam.SourceAssetHash != cntmCcntmract && lockParam.SourceAssetHash != cntgCcntmract {
		return utils.BYTE_FALSE, fmt.Errorf("[Lock] only support cntm/cntg lock, expect:%s or %s, but got:%s", hex.EncodeToString(cntmCcntmract[:]), hex.EncodeToString(cntgCcntmract[:]), hex.EncodeToString(lockParam.SourceAssetHash[:]))
	}

	// transfer cntm or cntg from FromAddress to lockCcntmract
	state := cntm.State{
...
func Unlock(native *native.NativeService) ([]byte, error) {
	...
	proxyCcntmractHash, err := utils.GetStorageVarBytes(native, GenBindProxyKey(contract, unlockParam.FromChainId))
	if err != nil {
	...
	if !bytes.Equal(proxyCcntmractHash, unlockParam.FromCcntmractHashBs) {
		return utils.BYTE_FALSE, fmt.Errorf("[Unlock] passed in contractHash NOT equal stored contractHash with chainID:%d, expect:%s, got:%s", unlockParam.FromChainId, hex.EncodeToString(proxyCcntmractHash), hex.EncodeToString(unlockParam.FromCcntmractHashBs))
	}

	// currently, only support cntm and cntg unlock operation
	if !bytes.Equal(args.TargetAssetHash, cntmCcntmract[:]) && !bytes.Equal(args.TargetAssetHash, cntgCcntmract[:]) {
		return utils.BYTE_FALSE, fmt.Errorf("[Unlock] target asset hash, Is CNTM contract?:%t, Is CNTG contract?:%t, Args.TargetAssetHash:%s", bytes.Equal(args.TargetAssetHash, cntmCcntmract[:]), bytes.Equal(args.ToAddress, cntgCcntmract[:]), hex.EncodeToString(args.TargetAssetHash))
	}
	...
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("[Unlock] parse from Args.ToAddress to cntm address format error:%s", err)
	}
	if args.Value == 0 {
		return utils.BYTE_TRUE, nil
	}
	// unlock cntm or cntg from current proxy contract into toAddress
	transferInput := getTransferInput(cntm.State{contract, toAddress, args.Value})
	...
	AddUnLockNotifications(native, contract, unlockParam.FromChainId, unlockParam.FromCcntmractHashBs, assetAddress, toAddress, args.Value)
...
func WithdrawCNTG(native *native.NativeService) ([]byte, error) {
	...
	if eof {
		return utils.BYTE_FALSE, fmt.Errorf("[WithdrawCNTG] input DecodeAddress toAddress error!")
	}
	operatorAddress, err := types.AddressFromBookkeepers(genesis.GenesisBookkeepers)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("[WithdrawCNTG] get operator error: %v", err)
	}
	//check witness
	if err = utils.ValidateOwner(native, operatorAddress); err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("[WithdrawCNTG] checkWitness error: %v", err)
	}
	// query unbound cntg or allowance
	allowanceInput := getAllowanceInput()
	allowanceRes, err := native.NativeCall(utils.CntgCcntmractAddress, cntm.ALLOWANCE_NAME, allowanceInput)
	if err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("[WithdrawCNTG] invoke cntg contract get allowance error:%s", err)
	}
	allowance := common.BigIntFromCntmBytes(allowanceRes)
	// transfer cntg to toAddress
	transferFromInput := getTransferFromInput(toAddress, allowance.Uint64())
	if _, err = native.NativeCall(utils.CntgCcntmractAddress, cntm.TRANSFERFROM_NAME, transferFromInput); err != nil {
		return utils.BYTE_FALSE, fmt.Errorf("[WithdrawCNTG] invoke cntg contract, transferFrom(lockProxy, cntmCcntmract, toAddress, unboundCntgAmount) err:%s", err)
	}
	if config.DefConfig.Common.EnableEventLog {
		native.Notifications = append(native.Notifications,
			&event.NotifyEventInfo{
				CcntmractAddress: contract,
				States:          []interface{}{WITHDRAW_CNTG_NAME, toAddress.ToBase58(), allowance.Uint64()},
			})
	}
	return utils.BYTE_TRUE, nil
}
...
func GetCrossedAmount(native *native.NativeService) ([]byte, error) {
	...
	return common.BigIntToCntmBytes(big.NewInt(0).SetBytes(crossedAmountBs)), nil
}
...
func GetCrossedLimit(native *native.NativeService) ([]byte, error) {
	...
	return common.BigIntToCntmBytes(big.NewInt(0).SetBytes(crossedLimitBs)), nil
}
