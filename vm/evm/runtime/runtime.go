...
func Execute(code, input []byte, cfg *Config) ([]byte, *storage.StateDB, error) {
	...
	var (
		address = common.BytesToAddress([]byte("ccntmract"))
		vmenv   = NewEnv(cfg)
	...
	ret, _, err := vmenv.Call(
		sender,
		common.BytesToAddress([]byte("ccntmract")),
		input,
		cfg.GasLimit,
		cfg.Value,
	)
...
// Call executes the code given by the ccntmract's address. It will return the
