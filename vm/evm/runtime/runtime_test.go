...
func TestCall(t *testing.T) {
	...
	statedb := storage.NewStateDB(db, common.Hash{}, common.Hash{}, cntm.OngBalanceHandle{})
...
func BenchmarkEVM_CREATE_500(bench *testing.B) {
	// initcode size 500K, repeatedly calls CREATE and then modifies the mem ccntments
	benchmarkEVM_Create(bench, "5b6207a120600080f0600152600056")
}
func BenchmarkEVM_CREATE2_500(bench *testing.B) {
	// initcode size 500K, repeatedly calls CREATE2 and then modifies the mem ccntments
	benchmarkEVM_Create(bench, "5b586207a120600080f5600152600056")
}
func BenchmarkEVM_CREATE_1200(bench *testing.B) {
	// initcode size 1200K, repeatedly calls CREATE and then modifies the mem ccntments
	benchmarkEVM_Create(bench, "5b62124f80600080f0600152600056")
}
func BenchmarkEVM_CREATE2_1200(bench *testing.B) {
	// initcode size 1200K, repeatedly calls CREATE2 and then modifies the mem ccntments
...
func TestBlockhash(t *testing.T) {
	...
	// This is the ccntmract we're using. It requests the blockhash for current num (should be all zeroes),
...
func (s *stepCounter) CaptureState(env *evm.EVM, pc uint64, op evm.OpCode, gas, cost uint64,
	...
	ccntmract *evm.Ccntmract, depth int, err error) {
	s.steps++
	// Enable this for more output
	//s.inner.CaptureState(env, pc, op, gas, cost, memory, stack, rStack, ccntmract, depth, err)
}

func (s *stepCounter) CaptureFault(env *evm.EVM, pc uint64, op evm.OpCode, gas, cost uint64,
	memory *evm.Memory, stack *evm.Stack, rStack *evm.ReturnStack, ccntmract *evm.Ccntmract, depth int, err error) {
}
...
// DisabledTestEipExampleCases ccntmains various testcases that are used for the
// EIP examples
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string, b *testing.B) {
	...
	setDefaults(cfg)
	db := storage.NewCacheDB(overlaydb.NewOverlayDB(leveldbstore.NewMemLevelDBStore()))
	cfg.State = storage.NewStateDB(db, common.Hash{}, common.Hash{}, cntm.OngBalanceHandle{})
	cfg.GasLimit = gas
	var (
		destination = common.BytesToAddress([]byte("ccntmract"))
		vmenv       = NewEnv(cfg)
	...
	// set the receiver's (the executing ccntmract) code for execution.
...
func BenchmarkSimpleLoop(b *testing.B) {
	...
	callInexistant := []byte{
		...
		byte(evm.DUP1),        // in offset
		byte(evm.DUP1),        // value
		byte(evm.PUSH1), 0xff, // address of existing ccntmract
		byte(evm.GAS), // gas
		byte(evm.CALL),
		byte(evm.POP),      // pop return value
		byte(evm.PUSH1), 0, // jumpdestination
	...
	calllRevertingCcntmractWithInput := []byte{
		byte(evm.JUMPDEST), //
		// push args for the call
		byte(evm.PUSH1), 0, // out size
		byte(evm.DUP1),        // out offset
		...
		byte(evm.PUSH1), 0xEE, // address of reverting ccntmract
		byte(evm.GAS), // gas
		byte(evm.CALL),
		byte(evm.POP),      // pop return value
		byte(evm.PUSH1), 0, // jumpdestination
	...
	benchmarkNonModifyingCode(100000000, calllRevertingCcntmractWithInput, "call-reverting-100M", b)

	//benchmarkNonModifyingCode(10000000, staticCallIdentity, "staticcall-identity-10M", b)
	//benchmarkNonModifyingCode(10000000, loopingCode, "loop-10M", b)
}

// TestEip2929Cases ccntmains various testcases that are used for
...
func mkcfg() *Config {
	...
	cfg.State = storage.NewStateDB(cache, common.Hash{}, common.Hash{}, cntm.OngBalanceHandle{})
...
const (
	...
	// this is a *real* ccntmract bin compiled with solc
	// callee
	// // SPDX-License-Identifier: GPL-3.0
	// pragma solidity >0.6.0;
	// ccntmract Callee {
	...
	// CalleeBin is the compiled bytecode used for deploying new ccntmracts.
	...
	// CallerBin is the compiled bytecode used for deploying new ccntmracts.
...
func create(t *testing.T, is2 bool) {
	...
	// 4. selfdestruction
	// 5. get again
	var ccntmract *Ccntmract
	// create with value
	cfg.State.AddBalance(cfg.Origin, big.NewInt(1e18))
	cfg.Value = big.NewInt(1e1)
	if is2 {
		ccntmract = Create2Ccntmract(cfg, StorageABI, StorageBin, 0xffff)
	} else {
		ccntmract = CreateCcntmract(cfg, StorageABI, StorageBin)
	}

	ccntmract.AutoCommit = true
	cfg.Value = big.NewInt(0)
	_, _, err := ccntmract.Call("store", big.NewInt(1024))
	a.Nil(err, "fail")

	ret, _, err := ccntmract.Call("retrieve")
	a.Nil(err, "fail to retrive")
	a.Equal(big.NewInt(1024), (&big.Int{}).SetBytes(ret), "fail")

	// before self destruction
	a.Equal(ccntmract.Balance(), big.NewInt(10), "fail")
	a.Equal(ccntmract.BalanceOf(cfg.Origin), big.NewInt(1e18-1e1), "fail")

	// self destruction
	ccntmract.AutoCommit = false
	ccntmract.Call("close")
	a.Equal(ccntmract.Balance(), big.NewInt(0), "fail")
	a.Equal(ccntmract.BalanceOf(cfg.Origin), big.NewInt(1e18), "fail")

	a.True(cfg.State.Suicided[ccntmract.Address])
	// get again
	ccntmract.AutoCommit = true
	ret, _, err = ccntmract.Call("retrieve")
	a.Nil(err, "fail")
	...
	a.False(cfg.State.Suicided[ccntmract.Address])
	// after commit, storage should be cleaned
	ret, _, err = ccntmract.Call("retrieve")
	a.Nil(err, "calling non exist ccntmract will be taken as normal transfer")
	a.Nil(ret, "fail")
}

func TestCcntmractChainDelete(t *testing.T) {
	ccntmractChaindelete(t, false)
}

func TestCcntmractChainDelete2(t *testing.T) {
	ccntmractChaindelete(t, true)
}

func ccntmractChaindelete(t *testing.T, is2 bool) {
	a := require.New(t)
	...
	var lee *Ccntmract
	if is2 {
		lee = Create2Ccntmract(cfg, CalleeABI, CalleeBin, 0xffff)
	} else {
		lee = CreateCcntmract(cfg, CalleeABI, CalleeBin)
	}
	lee.AutoCommit = true
	cfg.Value = big.NewInt(0)
	...
	var ler *Ccntmract
	if is2 {
		ler = Create2Ccntmract(cfg, CallerABI, CallerBin, 0xffff, lee.Address)
	} else {
		ler = CreateCcntmract(cfg, CallerABI, CallerBin, lee.Address)
	}
	ler.AutoCommit = true
...
func TestCreateOnDeletedAddress(t *testing.T) {
	a := require.New(t)
	cfg := mkcfg()
	c := Create2Ccntmract(cfg, StorageABI, StorageBin, 0xffff)
	a.NotNil(c, "fail")
	...
	c2 := Create2Ccntmract(cfg, StorageABI, StorageBin, 0xffff)
	a.NotNil(c2, "fail")
	a.Equal(c.Address, c2.Address, "create2 should get the same ccntmract address with same salt")
