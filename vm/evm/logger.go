...
// Storage represents a ccntmract's storage.
...
type Tracer interface {
	...
	CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack,
		rStack *ReturnStack, rData []byte, ccntmract *Ccntmract, depth int, err error)
	CaptureFault(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack,
		rStack *ReturnStack, ccntmract *Ccntmract, depth int, err error)
...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory,
	stack *Stack, rStack *ReturnStack, rData []byte, ccntmract *Ccntmract, depth int, err error) {
	...
	// Copy a snapshot of the current storage to a new ccntmainer
	var storage Storage
	if !l.cfg.DisableStorage {
		// initialise new changed values storage ccntmainer for this ccntmract
		// if not present.
		if l.storage[ccntmract.Address()] == nil {
			l.storage[ccntmract.Address()] = make(Storage)
		}
		// capture SLOAD opcodes and record the read entry in the local storage
		if op == SLOAD && stack.len() >= 1 {
			var (
				address = common.Hash(stack.data[stack.len()-1].Bytes32())
				value   = env.StateDB.GetState(ccntmract.Address(), address)
			)
			l.storage[ccntmract.Address()][address] = value
		}
		...
		storage = l.storage[ccntmract.Address()].Copy()
	}
	var rdata []byte
...
func (l *StructLogger) CaptureFault(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory,
	stack *Stack, rStack *ReturnStack, ccntmract *Ccntmract, depth int, err error) {
}
