...
from cntmology.interop.System.Runtime import CheckWitness
...
def init():
    supply = Get(GetCcntmext(), TOTAL_SUPPLY_KEY)
    assert(supply == 0)
    total = TotalSupply * FACTOR
    Put(GetCcntmext(), TOTAL_SUPPLY_KEY, total)
    Put(GetCcntmext(), concat(BALANCE_PREFIX, Admin), total)
