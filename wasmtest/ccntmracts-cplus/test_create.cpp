...
class hello: public ccntmract {
	...
	int64_t test_ccntmract_create(void) {
		...
		address t = cntmio::ccntmract_create(code, 3, "name", "version", "author", "email", "desc");
		vector<char> args = {};
	...
	string testcase(void) {
		return string(R"(
		[
			[{"method":"test_ccntmract_create", "param":"", "expected":""}
			]
		]
		)");
	}
...
cntmIO_DISPATCH( hello, (testcase)(test_ccntmract_create))
