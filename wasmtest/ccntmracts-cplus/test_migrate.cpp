/*test multi storage value should be move to new ccntmract address*/
...
class hello: public ccntmract {
	...
	int64_t test_ccntmract_migrate(void) {
		...
		address t = cntmio::ccntmract_migrate(code, 3, "name", "version", "author", "email", "desc");
		vector<char> args = {};
	...
	string testcase(void) {
		...
		[
			...
			{"method":"test_ccntmract_migrate", "param":"", "expected":""}
			]
		]
		)");
	}
...
cntmIO_DISPATCH( hello, (testcase)(test_ccntmract_migrate)(testStorage)(testStorage2))
