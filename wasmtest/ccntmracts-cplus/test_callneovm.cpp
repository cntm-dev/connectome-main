...
class hello: public ccntmract {
	...
	address callneo_address(test_conext &tc) {
		...
		call_neo_ccntmract(test_neo, neoargs, res);
		address res_addr;
		std::copy(res.begin(), res.end(), res_addr.begin());
		check(res_addr == test_neo, "get bytearray result wrcntm");
		return res_addr;
	}
	...
	bool callneo_bool(test_conext &tc) {
		...
		auto neoargs = serialize_args_forneo(arg0, arg1);
		call_neo_ccntmract(test_neo, neoargs, res);
		check(res.value == arg1.value, "get bool result wrcntm");
		return res.value;
	}
	...
	void callneo_intype(test_conext &tc) {
		...
		auto neoargs = serialize_args_forneo(arg0, arg1);
		call_neo_ccntmract(test_neo, neoargs, res);
		check(res.value == arg1.value + 0x101, "get int64_t result wrcntm");
	...
	void callneo_H256(test_conext &tc) {
		...
		NeoByteArray resl;
		auto neoargs = serialize_args_forneo(arg0, arg1);
		call_neo_ccntmract(test_neo, neoargs, resl);
		NeoH256 res;
		std::copy(resl.begin(), resl.end(), res.value.begin());
		check(res.value == arg1.value, "get wrcntm h256");
	}
	...
	string testcase(void) {
		...
		[
			...
			{"needccntmext":true,"env":{"witness":[]}, "method":"callneo_intype", "param":"", "expected":""},
			{"needccntmext":true,"env":{"witness":[]}, "method":"callneo_H256", "param":"", "expected":""},
			{"needccntmext":true,"env":{"witness":[]}, "method":"callneo_listype", "param":"", "expected":""}
    	    ]
		]
		)");
	}
...
cntmIO_DISPATCH( hello, (testcase)(callneo_address)(callneo_bool)(callneo_intype)(callneo_H256)(callneo_listype))
