...
An Example of OEP-4
"""
from cntmology.interop.System.Storage import GetCcntmext, Get, Put, Delete
from cntmology.interop.System.Runtime import Notify, CheckWitness
...
def Main(operation, args):
    ...
    :return:
    """
    # 'init' has to be invokded first after deploying the ccntmract to store the necessary and important info in the blockchain
    if operation == 'init':
        return init()
    if operation == 'name':
        return name()
...
def init():
    """
    initialize the ccntmract, put some important info into the storage in the blockchain
    :return:
    """
...
def transferMulti(args):
    """
    :param args: the parameter is an array, ccntmaining element like [from, to, amount]
    :return: True means success, False or raising exception means failure.
    """
    for p in args:
        if len(p) != 3:
            # return False is wrcntm
            raise Exception("transferMulti params error.")
        if transfer(p[0], p[1], p[2]) == False:
            # return False is wrcntm since the previous transaction will be successful
