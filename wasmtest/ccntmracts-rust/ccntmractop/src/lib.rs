...
use ostd::prelude::*;
use ostd::runtime;
use ostd::ccntmract::cntm;

extern crate alloc;
use alloc::collections::BTreeMap;
use cntmio_std::console::debug;

pub struct TestCcntmext<'a> {
    admin: &'a Address,
    ...
    match action {
        b"test_migrate" => {
            ...
            (module
              ...
              (import "env" "cntmio_return" (func (;0;) (type 1)))
              (import "env" "cntmio_storage_read" (func (;1;) (type 2)))
              (func (;2;) (type 0)
            ...
            let address =
                runtime::ccntmract_migrate(code, 3, "name", "version", "author", "email", "desc")
                    .expect("migrate failed");
            let resv = runtime::call_ccntmract(&address, &[]).expect("call_ccntmract failed");
        ...
        b"test_callwasm" => {
            ...
            let resv = runtime::call_ccntmract(&address, isink.bytes()).expect("get no return");
            runtime::ret(&resv);
            return;
        }
        b"test_calljsvm" => {
        ...
        b"balanceOf" => {
            let balance = cntm::balance_of(&runtime::address());
            sink.write(balance)
        }
        b"transferFromAdmin" => {
            let amount = source.read().unwrap();
            let tc = get_tc(&mut source);
            sink.write(cntm::transfer(tc.admin, &runtime::address(), amount))
        }
        b"transferToAdmin" => {
            let amount = source.read().unwrap();
            let tc = get_tc(&mut source);
            sink.write(cntm::transfer(&runtime::address(),tc.admin, amount))
        }
        b"approveAndTransferFromAdmin" => {
            let amount = source.read().unwrap();
            let tc = get_tc(&mut source);
            let res = cntm::approve(tc.admin, &runtime::address(), amount);
            assert_eq!(res, true);
            let allo = cntm::allowance(tc.admin, &runtime::address());
            assert_eq!(allo, amount);
            sink.write(cntm::transfer_from(&runtime::address(), tc.admin, &runtime::address(), amount))
        }
...
fn get_tc<'a>(source: &mut Source<'a>) -> TestCcntmext<'a> {
...
fn testcase() -> String {
    ...
    [
        ...
        {"method":"transferFromAdmin", "needccntmext":true, "param":"int:100", "expected":"bool:true"},
        {"method":"balanceOf", "expected":"int:100"},
        {"method":"approveAndTransferFromAdmin", "needccntmext":true, "param":"int:100", "expected":"bool:true"},
        {"method":"balanceOf", "expected":"int:200"},
        {"method":"transferToAdmin", "needccntmext":true, "param":"int:200", "expected":"bool:true"},
        {"method":"balanceOf", "expected":"int:0"},

        {"method":"storage_write"},
        {"method":"storage_write2"},
        {"needccntmext":true, "env":{"witness":[]}, "method":"test_callwasm", "param":"string:add, int:1, int:2", "expected":"int:3"},
        {"needccntmext":true, "env":{"witness":[]}, "method":"test_calljsvm", "param":"string:evaluate,string:function fib(n) {if(n<=0){return 1;} else if (n == 1) {return 1;} else { return fib(n-1) + fib(n-2);}} fib(3)", "expected":"string:3"},
        {"needccntmext":true, "env":{"witness":[]}, "method":"test_calljsvm", "param":"string:evaluate,string:function sum(a) {var sum = 0;var i = 0;while (i<a) {sum = sum + i;i = i + 1;} return sum;};sum(5);", "expected":"string:10"},
        {"needccntmext":true, "env":{"witness":[]}, "method":"test_calljsvm", "param":"string:evaluate,string:function mul(a) { return a*a;}; function sum(n) { let sum = 0; let i = 0; while( i<n) { sum = sum + mul(i);i = i + 1}; return sum;};sum(10);", "expected":"string:285"},
        {"method":"test_migrate"}
        ]
    ]
        "#
