...
import (
	...
	common2 "github.com/cntmio/cntmology/core/store/common"
	...
	types2 "github.com/cntmio/cntmology/http/ethrpc/types"
	utils2 "github.com/cntmio/cntmology/http/ethrpc/utils"
...
func getOngBalance(address common.Address) (states.NativeTokenBalance, error) {
	balances, _, err := hComm.GetNativeTokenBalance(0, []oComm.Address{utils.OngCcntmractAddress}, oComm.Address(address), true)
	if err != nil {
		return states.NativeTokenBalance{}, fmt.Errorf("get cntm balance error:%s", err)
	}
...
func (api *EthereumAPI) SendRawTransaction(data hexutil.Bytes) (common.Hash, error) {
	...
	if errCode, desc := hComm.SendTxToPool(eip155tx); errCode != cntmErrors.ErrNoError {
		if errCode == cntmErrors.ErrDuplicatedTx {
...
func generateRecipient(notify *event.ExecuteNotify) (interface{}, error) {
	...
	receipt := map[string]interface{}{
		...
		// sender and receiver (ccntmract or EOA) addresses
		"from": from,
		"to":   eip155Tx.To(),
	}
	if logs == nil {
	...
	ethCcntmractAddr := common.Address(notify.CreatedCcntmract)
	// If the CcntmractAddress is 20 0x0 bytes, assume it is not a ccntmract creation
	if ethCcntmractAddr != (common.Address{}) {
		receipt["ccntmractAddress"] = ethCcntmractAddr
	}
	return receipt, nil
}
