...
func GetCcntmractStateFromStore(hash common.Address) (*payload.DeployCode, error) {
	future := defLedgerPid.RequestFuture(&lactor.GetCcntmractStateReq{hash}, ReqTimeout*time.Second)
	result, err := future.Result()
	if err != nil {
		log.Errorf(ErrActorComm, err)
		return nil, err
	}
	if rsp, ok := result.(*lactor.GetCcntmractStateRsp); !ok {
		log.Error(ErrActorComm,"GetCcntmractStateRsp")
		return nil, errors.New("fail")
	} else {
		return rsp.CcntmractState, rsp.Error
	}
}
...
func PreExecuteCcntmract(tx *types.Transaction) ([]interface{}, error) {
	future := defLedgerPid.RequestFuture(&lactor.PreExecuteCcntmractReq{tx}, ReqTimeout*time.Second)
	result, err := future.Result()
	if err != nil {
		log.Errorf(ErrActorComm, err)
		return nil, err
	}
	if rsp, ok := result.(*lactor.PreExecuteCcntmractRsp); !ok {
		return nil, errors.New("fail")
	} else {
