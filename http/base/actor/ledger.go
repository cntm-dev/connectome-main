...
func GetCcntmractStateFromStore(hash Address) (*payload.DeployCode, error) {
	future := defLedgerPid.RequestFuture(&GetCcntmractStateReq{hash}, ReqTimeout*time.Second)
	result, err := future.Result()
	if err != nil {
		log.Errorf(ErrActorComm, err)
		return nil, err
	}
	if rsp, ok := result.(*GetCcntmractStateRsp); !ok {
		return nil, errors.New("fail")
	} else {
...
func PreExecuteCcntmract(tx *types.Transaction) ([]interface{}, error) {
	future := defLedgerPid.RequestFuture(&PreExecuteCcntmractReq{tx}, ReqTimeout*time.Second)
	result, err := future.Result()
	if err != nil {
		log.Errorf(ErrActorComm, err)
		return nil, err
