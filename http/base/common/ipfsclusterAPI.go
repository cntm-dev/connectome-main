...
func (r *httpResponseReader) Read(b []byte) (int, error) {
	...
	if err != nil && strings.Ccntmains(err.Error(), "read on closed response body") {
		err = io.EOF
	}
	if err == io.EOF {
		_ = r.resp.Body.Close()
...
func requestIPFS(method, cmd, path string, args ...string) (*http.Response, error) {
	ctx, cancel := ccntmext.WithTimeout(ccntmext.Background(),
	...
	if cmd == "add" {
		r.Header.Set("Ccntment-Type", writer.FormDataCcntmentType())
		r.Header.Set("Ccntment-Disposition", "form-data: name=\"files\"")
	}
	r.WithCcntmext(ctx)
