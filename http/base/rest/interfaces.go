...
func SendRawTransaction(cmd map[string]interface{}) map[string]interface{} {
	...
	if txn.TxType == types.Invoke || txn.TxType == types.Deploy {
		if preExec, ok := cmd["PreExec"].(string); ok && preExec == "1" {
			rst, err := bactor.PreExecuteCcntmract(txn)
			if err != nil {
...
//get smartccntmract event by height
...
//get smartccntmract event by transaction hash
...
func GetCcntmractState(cmd map[string]interface{}) map[string]interface{} {
	...
	ccntmract, err := bactor.GetCcntmractStateFromStore(address)
	if err != nil {
		return ResponsePack(berr.INTERNAL_ERROR)
	}
	if ccntmract == nil {
