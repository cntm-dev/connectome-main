...
const (
	...
	GET_CcntmRACT_STATE    = "/api/v1/ccntmract/:hash"
...
func (this *restServer) registryMethod() {
	...
	getMethodMap := map[string]Action{
		...
		GET_CcntmRACT_STATE:    {name: "getccntmract", handler: rest.GetCcntmractState},
...
func (this *restServer) getPath(url string) string {
	...
	if strings.Ccntmains(url, strings.TrimRight(GET_BLK_TXS_BY_HEIGHT, ":height")) {
		return GET_BLK_TXS_BY_HEIGHT
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BLK_BY_HEIGHT, ":height")) {
		return GET_BLK_BY_HEIGHT
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BLK_HASH, ":height")) {
		return GET_BLK_HASH
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BLK_BY_HASH, ":hash")) {
		return GET_BLK_BY_HASH
	} else if strings.Ccntmains(url, strings.TrimRight(GET_TX, ":hash")) {
		return GET_TX
	} else if strings.Ccntmains(url, strings.TrimRight(GET_CcntmRACT_STATE, ":hash")) {
		return GET_CcntmRACT_STATE
	} else if strings.Ccntmains(url, strings.TrimRight(GET_SMTCOCE_EVT_TXS, ":height")) {
		return GET_SMTCOCE_EVT_TXS
	} else if strings.Ccntmains(url, strings.TrimRight(GET_SMTCOCE_EVTS, ":hash")) {
		return GET_SMTCOCE_EVTS
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BLK_HGT_BY_TXHASH, ":hash")) {
		return GET_BLK_HGT_BY_TXHASH
	} else if strings.Ccntmains(url, strings.TrimRight(GET_STORAGE, ":hash/:key")) {
		return GET_STORAGE
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BALANCE, ":addr")) {
		return GET_BALANCE
	} else if strings.Ccntmains(url, strings.TrimRight(GET_BALANCE_V2, ":addr")) {
		return GET_BALANCE_V2
	} else if strings.Ccntmains(url, strings.TrimRight(GET_MERKLE_PROOF, ":hash")) {
		return GET_MERKLE_PROOF
	} else if strings.Ccntmains(url, strings.TrimRight(GET_ALLOWANCE, ":asset/:from/:to")) {
		return GET_ALLOWANCE
	} else if strings.Ccntmains(url, strings.TrimRight(GET_ALLOWANCE_V2, ":asset/:from/:to")) {
...
func (this *restServer) Stop() {
	if this.server != nil {
		this.server.Shutdown(ccntmext.Background())
