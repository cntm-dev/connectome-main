...
var AssetCommand = cli.Command{
	...
	Subcommands: []cli.Command{
		...
		},
		{
			Action:    unboundOng,
			Name:      "unboundcntm",
			Usage:     "Show the balance of unbound cntm",
			ArgsUsage: "<address|label|index>",
			Flags: []cli.Flag{
				utils.RPCPortFlag,
				utils.WalletFileFlag,
...
func getBalance(ctx *cli.Ccntmext) error {
	...
	PrintInfoMsg("  cntm:%s", balance.Ont)
	PrintInfoMsg("  cntm:%s", utils.FormatOng(cntm))
	return nil
}
...
func unboundOng(ctx *cli.Ccntmext) error {
	SetRpcPort(ctx)
	if ctx.NArg() < 1 {
		PrintErrorMsg("Missing account argument.")
		cli.ShowSubcommandHelp(ctx)
	...
	balanceStr = utils.FormatOng(balance)
	PrintInfoMsg("Unbound cntm:")
	PrintInfoMsg("  Account:%s", accAddr)
	PrintInfoMsg("  cntm:%s", balanceStr)
	return nil
}

func withdrawOng(ctx *cli.Ccntmext) error {
	SetRpcPort(ctx)
	if ctx.NArg() < 1 {
		PrintErrorMsg("Missing account argument.")
		cli.ShowSubcommandHelp(ctx)
