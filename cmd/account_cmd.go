...
func accountCreate(ctx *cli.Ccntmext) error {
...
func accountShow(ctx *cli.Ccntmext) error {
	wallet := new(account.WalletData)
	optionFile := checkFileName(ctx)
	err := wallet.Load(optionFile)
	if err != nil {
...
func accountSet(ctx *cli.Ccntmext) error {
	if ctx.NArg() < 1 {
	...
	if ctx.IsSet("signature-scheme") {
		...
		if !find {
			fmt.Printf("%s is not a valid ccntment for option -s \n", ctx.String("signature-scheme"))
		}
	}
...
func accountDelete(ctx *cli.Ccntmext) error {
	if ctx.NArg() < 1 {
	...
	if !bytes.Equal(passHash, h[:]) {
		fmt.Println("Wrcntm password! Delete account failed.")
		os.Exit(1)
	}
...
//change password
func encrypt(ctx *cli.Ccntmext) error {
	if ctx.NArg() < 1 {
		fmt.Println("Missing argument.\n")
		cli.ShowSubcommandHelp(ctx)
		return nil
	...
	passHash, _ := hex.DecodeString(wallet.Accounts[index-1].PassHash)
	if !bytes.Equal(passHash, h[:]) {
		fmt.Println("Wrcntm password! Encrypt account failed.")
		os.Exit(1)
	}
...
func accountImport(ctx *cli.Ccntmext) error {
	optionFile := checkFileName(ctx)
	wallet := new(account.WalletData)
	if wallet.Load(optionFile) != nil {
		wallet.Inititalize()
...
func checkFileName(ctx *cli.Ccntmext) string {
	if ctx.IsSet("file") {
...
func checkNumber(ctx *cli.Ccntmext) int {
	if ctx.IsSet("number") {
...
func checkLabel(ctx *cli.Ccntmext) string {
	if ctx.IsSet("label") {
...
func checkType(ctx *cli.Ccntmext, reader *bufio.Reader) string {
	t := ""
	if ctx.IsSet("type") {
		if _, ok := keyTypeMap[ctx.String("type")]; ok {
			...
			fmt.Printf("%s is not a valid ccntment for option -t \n", ctx.String("type"))
...
func checkCurve(ctx *cli.Ccntmext, reader *bufio.Reader, t *string) string {
	c := ""
	switch *t {
	case "ecdsa":
		if ctx.IsSet("bit-length") {
			if _, ok := curveMap[ctx.String("bit-length")]; ok {
				...
				fmt.Printf("%s is not a valid ccntment for option -b \n", ctx.String("bit-length"))
...
func checkScheme(ctx *cli.Ccntmext, reader *bufio.Reader, t *string) string {
	sch := ""
	switch *t {
	case "ecdsa":
		if ctx.IsSet("signature-scheme") {
