...
func accountCreate(ctx *cli.Ccntmext) error {
	...
	if ctx.Bool(utils.IdentityFlag.Name) {
		...
		if err != nil {
			return fmt.Errorf("Create cntm ID error: %s", err)
		}
		wd.AddIdentity(id)
		...
		fmt.Println("")
		fmt.Println("cntm ID created:", id.ID)
		fmt.Println("Bind public key:", id.Ccntmrol[0].Public)
		return nil
	}
...
func accountList(ctx *cli.Ccntmext) error {
	optionFile := checkFileName(ctx)
	wallet, err := account.Open(optionFile)
	if err != nil {
...
func accountExport(ctx *cli.Ccntmext) error {
	if ctx.NArg() <= 0 {
...
func genMultiAddress(ctx *cli.Ccntmext) error {
