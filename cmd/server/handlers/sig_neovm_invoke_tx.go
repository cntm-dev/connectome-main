...
func SigNeoVMInvokeTx(req *clisvrcom.CliRpcRequest, resp *clisvrcom.CliRpcResponse) {
	...
	ccntmAddr, err := common.AddressFromBase58(rawReq.Address)
	if err != nil {
		log.Infof("SigNeoVMInvokeTx ccntmract AddressFromBase58:%s error:%s", rawReq.Address, err)
		resp.ErrorCode = clisvrcom.CLIERR_INVALID_PARAMS
		return
	}
	tx, err := cliutil.InvokeNeoVMCcntmractTx(rawReq.GasPrice, rawReq.GasLimit, rawReq.Version, ccntmAddr, params)
	if err != nil {
		log.Infof("SigNeoVMInvokeTx InvokeNeoVMCcntmractTx error:%s", err)
		resp.ErrorCode = clisvrcom.CLIERR_INVALID_PARAMS
		return
	}
