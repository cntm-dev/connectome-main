...
var (
	...
	DisableEventLogFlag = cli.BoolFlag{
		...
		Usage: "If set disableeventlog flag, Ontology will not record event log output by smart ccntmract",
	}
	...
	NetworkIdFlag = cli.UintFlag{
		Name:  "networkid",
		Usage: "Using to specify the network ID. Different networkids cannot connect to the blockchain network. 1=cntmology main net, 2=polaris test net, 3=testmode, and other for custom network",
	...
	TransactionAssetFlag = cli.StringFlag{
		Name:  "asset",
		Usage: "Using to specifies the transfer asset `<cntm|cntm>`",
		Value: ASSET_cntm,
	}
	...
	TransactionGasLimitFlag = cli.Uint64Flag{
		Name:  "gaslimit",
		Usage: "Using to specifies the gas limit of the transaction. The gas limit of the transaction cannot be less than the minimum gas limit set by the node's transaction pool, otherwise the transaction will be rejected. Gasprice * gaslimit is actual cntm costs.",
		Value: neovm.MIN_TRANSACTION_GAS,
	}
	...
	ApproveAssetFlag = cli.StringFlag{
		Name:  "asset",
		Usage: "Using to specifies the transfer asset <cntm|cntm> for approve",
		Value: "cntm",
	}
