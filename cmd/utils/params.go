...
//Note that if string ccntmain some special char like :,[,] and so one, please '/' char to escape. For example: string:did/:ed1e25c9dccae0c694ee892231407afa20b76008
...
func parasReturnValueWasmArray(rawValues []interface{}, returnTypes []interface{}) ([]interface{}, error) {
	...
	for i := 0; i < len(rawValues); i++ {
		...
		case string:
			...
			case PARAM_TYPE_BYTE_ARRAY:
				value, err = ParseWasmVMCcntmractReturnTypeByteArray(v)
			case PARAM_TYPE_STRING:
				value, err = ParseWasmVMCcntmractReturnTypeString(v)
			case PARAM_TYPE_INTEGER:
				value, err = ParseWasmVMCcntmractReturnTypeInteger(v)
			case PARAM_TYPE_BOOLEAN:
				value, err = ParseWasmVMCcntmractReturnTypeBool(v)
			default:
...
func parseReturnNeoValueArray(rawValues []interface{}, returnTypes []interface{}) ([]interface{}, error) {
	...
	for i := 0; i < len(rawValues); i++ {
		...
		case string:
			...
			switch strings.ToLower(vType) {
			case PARAM_TYPE_BYTE_ARRAY:
				value, err = ParseNeoVMCcntmractReturnTypeByteArray(v)
			case PARAM_TYPE_STRING:
				value, err = ParseNeoVMCcntmractReturnTypeString(v)
			case PARAM_TYPE_INTEGER:
				value, err = ParseNeoVMCcntmractReturnTypeInteger(v)
			case PARAM_TYPE_BOOLEAN:
				value, err = ParseNeoVMCcntmractReturnTypeBool(v)
			default:
				return nil, fmt.Errorf("unknown return type:%s", v)
			}
			values = append(values, value)
...
//NeoVMInvokeParam use to express the param to invoke neovm ccntmract.
