...
func NewNativeInvokeTransaction(gasPrice, gasLimit uint64, ccntmractAddr common.Address, version byte, params []interface{}, funcAbi *abi.NativeCcntmractFunctionAbi) (*types.Transaction, error) {
	...
	builder.EmitPushByteArray(ccntmractAddr[:])
	...
	return httpcom.NewSmartCcntmractTransaction(gasPrice, gasLimit, invokeCode)
}

func ParseNativeFuncParam(builder *neovm.ParamsBuilder, funName string, params []interface{}, paramsAbi []*abi.NativeCcntmractParamAbi) error {
	size := len(paramsAbi)
	if size == 0 {
		//Params cannot empty, if params is empty, fulfil with func name
		params = []interface{}{funName}
		paramsAbi = []*abi.NativeCcntmractParamAbi{{
			Name: "funcName",
			Type: abi.NATIVE_PARAM_TYPE_STRING,
		}}
	} else if size > 1 {
		//If more than one param in func, must using struct
		paramRoot := &abi.NativeCcntmractParamAbi{
			Name:    "root",
			Type:    abi.NATIVE_PARAM_TYPE_STRUCT,
			SubType: paramsAbi,
		}
		params = []interface{}{params}
		paramsAbi = []*abi.NativeCcntmractParamAbi{paramRoot}
	}
	return ParseNativeParams(builder, params, paramsAbi)
}

func ParseNativeParams(builder *neovm.ParamsBuilder, params []interface{}, paramsAbi []*abi.NativeCcntmractParamAbi) error {
...
func ParseNativeParamStruct(builder *neovm.ParamsBuilder, param interface{}, structAbi *abi.NativeCcntmractParamAbi) error {
	...
	for i, param := range params {
		...
		err := ParseNativeParams(builder, []interface{}{param}, []*abi.NativeCcntmractParamAbi{paramAbi})
		if err != nil {
...
func ParseNativeParamArray(builder *neovm.ParamsBuilder, param interface{}, arrayAbi *abi.NativeCcntmractParamAbi) error {
	params, ok := param.([]interface{})
	if !ok {
		return fmt.Errorf("assert to []interface{} failed")
	}
	abis := make([]*abi.NativeCcntmractParamAbi, 0, len(params))
	for i := 0; i < len(params); i++ {
		abis = append(abis, &abi.NativeCcntmractParamAbi{
...
func ParseNativeParamAddress(builder *neovm.ParamsBuilder, param string) error {
	...
	var err error
	//Maybe param is a ccntmract address
