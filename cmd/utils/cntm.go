...
const (
	...
	CcntmRACT_TRANSFER_V2   = "transferV2"
...
func ApproveTx(gasPrice, gasLimit uint64, asset string, from, to string, amount uint64) (*types.MutableTransaction, error) {
	...
	invokeCode, err := cutils.BuildNativeInvokeCode(ccntmractAddr, version, CcntmRACT_APPROVE, []interface{}{state})
	if err != nil {
...
func TransferTx(gasPrice, gasLimit uint64, asset, from, to string, amount uint64) (*types.MutableTransaction, error) {
	...
	invokeCode, err := cutils.BuildNativeInvokeCode(ccntmractAddr, version, CcntmRACT_TRANSFER, []interface{}{sts})
	if err != nil {
		return nil, fmt.Errorf("build invoke code error:%s", err)
	}
	mutableTx := NewInvokeTransaction(gasPrice, gasLimit, invokeCode)
...
func TransferTxV2(gasPrice, gasLimit uint64, asset, from, to string, amount *big.Int) (*types.MutableTransaction, error) {
	...
	var sts []*cntm.TransferStateV2
	sts = append(sts, &cntm.TransferStateV2{
		From:  fromAddr,
		To:    toAddr,
	...
	invokeCode, err := cutils.BuildNativeInvokeCode(ccntmractAddr, version, CcntmRACT_TRANSFER_V2, []interface{}{sts})
	if err != nil {
		return nil, fmt.Errorf("build invoke code error:%s", err)
	}
	mutableTx := NewInvokeTransaction(gasPrice, gasLimit, invokeCode)
...
func TransferFromTx(gasPrice, gasLimit uint64, asset, sender, from, to string, amount uint64) (*types.MutableTransaction, error) {
	...
	invokeCode, err := cutils.BuildNativeInvokeCode(ccntmractAddr, version, CcntmRACT_TRANSFER_FROM, []interface{}{transferFrom})
	if err != nil {
		return nil, fmt.Errorf("build invoke code error:%s", err)
	}
	mutableTx := NewInvokeTransaction(gasPrice, gasLimit, invokeCode)
...
func GetSmartCcntmractEvent(txHash string) (*httpcom.ExecuteNotify, error) {
...
func PrepareInvokeCodeNeoVMCcntmract(code []byte) (*httpcom.PreExecuteResult, error) {
...
//prepare invoke wasm
func PrepareInvokeWasmVMCcntmract(ccntmractAddress common.Address, params []interface{}) (*httpcom.PreExecuteResult, error) {
