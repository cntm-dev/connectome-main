...
import (
	...
	"github.com/cntmio/cntmology/p2pserver/dht"
...
func (self *Discovery) FindNodeHandle(ctx *p2p.Ccntmext, freq *types.FindNodeReq) {
	...
	if !self.maskSet.Has(remoteIP.String()) && !self.maskFilter.Ccntmains(remotePeer.Info.RemoteListenAddress()) {
		...
		for _, pair := range fresp.CloserPeers {
			...
			// hide mask node
			if self.maskSet.Has(ip) || self.maskFilter.Ccntmains(pair.Address) {
				ccntminue
			}
...
func (self *Discovery) FindNodeResponseHandle(ctx *p2p.Ccntmext, fresp *types.FindNodeResp) {
	if fresp.Success {
...
func (self *Discovery) AddrReqHandle(ctx *p2p.Ccntmext) {
	remotePeer := ctx.Sender()
...
func (self *Discovery) AddrHandle(ctx *p2p.Ccntmext, msg *types.Addr) {
	p2p := ctx.Network()
