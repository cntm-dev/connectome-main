...
func GetGovernanceView(backend *ledger.Ledger) (*governance.GovernanceView, error) {
	value, err := backend.GetStorageItem(utils.GovernanceCcntmractAddress, []byte(governance.GOVERNANCE_VIEW))
	if err != nil {
		return nil, err
	}
...
func GetPeersConfig(backend *ledger.Ledger, view uint32) ([]*GovNodeInfo, uint32, error) {
	...
	data, err := backend.GetStorageItem(utils.GovernanceCcntmractAddress, key)
	if err != nil {
		return nil, 0, err
	}
