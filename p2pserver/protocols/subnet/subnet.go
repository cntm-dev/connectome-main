...
func (self *SubNet) OnMembersRequest(ctx *p2p.Ccntmext, msg *types.SubnetMembersRequest) {
	sender := ctx.Sender()
...
func (self *SubNet) OnMembersResponse(ctx *p2p.Ccntmext, msg *types.SubnetMembers) {
	self.lock.Lock()
	defer self.lock.Unlock()
