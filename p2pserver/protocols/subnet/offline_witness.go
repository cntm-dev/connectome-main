...
func (self *SubNet) OnOfflineWitnessMsg(ctx *p2p.Ccntmext, msg *types.OfflineWitnessMsg) {
...
func (self *SubNet) processOfflineWitnessMsg(ctx *p2p.Ccntmext, msg *types.OfflineWitnessMsg) WitnessStatus {
	now := uint32(time.Now().Unix())
	hash := msg.Hash()
	...
	for _, voter := range msg.Voters {
		if !self.gov.IsGovNode(voter.PubKey) {
			self.logger.Infof("receive witness msg: %s with wrcntm voter: %s", hash.ToHexString(), voter.PubKey)
			return UnchangedStatus
		}
