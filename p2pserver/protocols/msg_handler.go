...
import (
	...
	"github.com/cntmio/cntmology/p2pserver/protocols/block_sync"
	"github.com/cntmio/cntmology/p2pserver/protocols/bootstrap"
	"github.com/cntmio/cntmology/p2pserver/protocols/discovery"
	"github.com/cntmio/cntmology/p2pserver/protocols/heatbeat"
	"github.com/cntmio/cntmology/p2pserver/protocols/recent_peers"
...
func (self *MsgHandler) HandlePeerMessage(ctx *p2p.Ccntmext, msg msgTypes.Message) {
	...
	case *msgTypes.Pcntm:
		self.heatBeat.PcntmHandle(ctx, m)
...
func HeadersReqHandle(ctx *p2p.Ccntmext, headersReq *msgTypes.HeadersReq) {
...
func (self *MsgHandler) blockHandle(ctx *p2p.Ccntmext, block *msgTypes.Block) {
...
func ConsensusHandle(ctx *p2p.Ccntmext, consensus *msgTypes.Consensus) {
...
func DataReqHandle(ctx *p2p.Ccntmext, dataReq *msgTypes.DataReq) {
	remotePeer := ctx.Sender()
...
func InvHandle(ctx *p2p.Ccntmext, inv *msgTypes.Inv) {
	remotePeer := ctx.Sender()
	if len(inv.P.Blk) == 0 {
	...
	case common.BLOCK:
		...
		for _, id = range inv.P.Blk {
			...
			// TODO check the ID queue
			isCcntmainBlock, err := ledger.DefLedger.IsCcntmainBlock(id)
			if err != nil {
				log.Warn(err)
				return
			}
