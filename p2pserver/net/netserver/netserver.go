...
func NewNetServer(protocol p2p.Protocol, conf *config.P2PNodeConfig, reserveAddrFilter p2p.AddressFilter) (*NetServer, error) {
	...
	option, err := connect_ccntmroller.ConnCtrlOptionFromConfig(conf, reserveAddrFilter)
	if err != nil {
		return nil, err
	}

	listener, err := connect_ccntmroller.NewListener(nodePort, conf)
	if err != nil {
...
func (this *NetServer) connect(addr string) error {
	...
	if err != nil {
		if err == connect_ccntmroller.ErrHandshakeSelf {
