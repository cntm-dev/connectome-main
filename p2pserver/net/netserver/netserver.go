...
func NewCustomNetServer(id *common.PeerKeyId, info *peer.PeerInfo, proto p2p.Protocol,
	listener net.Listener, opt connect_ccntmroller.ConnCtrlOption) *NetServer {
	n := &NetServer{
		base:       info,
		listener:   listener,
		protocol:   proto,
	...
	n.connCtrl = connect_ccntmroller.NewConnectCcntmroller(info, id, opt)
...
type NetServer struct {
	...
	connCtrl *connect_ccntmroller.ConnectCcntmroller
	...
	for {
		...
		case data, ok := <-channel:
			if ok {
				...
				ctx := p2p.NewCcntmext(sender, this, data.PayloadSize)
...
func (this *NetServer) init(conf *config.P2PNodeConfig) error {
	...
	option, err := connect_ccntmroller.ConnCtrlOptionFromConfig(conf)
	if err != nil {
		return err
	}
	this.connCtrl = connect_ccntmroller.NewConnectCcntmroller(this.base, keyId, option)
	...
	this.listener, err = connect_ccntmroller.NewListener(syncPort, config.DefConfig.P2PNode)
	if err != nil {
...
func (ns *NetServer) ConnectCcntmroller() *connect_ccntmroller.ConnectCcntmroller {
	return ns.connCtrl
}
