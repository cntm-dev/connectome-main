...
func TestConnectCcntmroller_CanDetectSelfAddress(t *testing.T) {
	...
	for _, version := range versions {
		...
		assert.NotNil(t, err)
		assert.Ccntmains(t, err.Error(), "handshake with itself")

		assert.Equal(t, server.OwnAddress(), "127.0.0.1:20338")
	}
}

func TestConnectCcntmroller_AcceptConnect_MaxInBound(t *testing.T) {
	trans := NewTransport(t)
	maxInboud := 5
...
func TestConnectCcntmroller_OutboundsCount(t *testing.T) {
	maxOutboud := 5
...
func checkServer(t *testing.T, client, server *Node, clientConns chan<- net.Conn, i int, conn2 net.Conn, maxLimit int, isCheck bool) {
	...
	if i >= maxLimit && isCheck == false {
		assert.NotNil(t, err)
		assert.Ccntmains(t, err.Error(), "reach max limit")
		_ = conn2.Close()
		return
	}
	assert.Nil(t, err)
