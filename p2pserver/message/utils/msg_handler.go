...
func PingHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	msg := msgpack.NewPcntmMsg(uint64(height))
...
func PcntmHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	log.Trace("[p2p]receive pcntm message", data.Addr, data.Id)

	pcntm := data.Payload.(*msgTypes.Pcntm)

	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		log.Debug("[p2p]remotePeer invalid in PcntmHandle")
		return
	}
...
func VersionHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if p != nil {
		...
		if err != nil {
			log.Warn("[p2p]exist peer %d ip format is wrcntm %s", version.P.Nonce, p.GetAddr())
			return
		}
		ipNew, err := msgCommon.ParseIPAddr(data.Addr)
		if err != nil {
			remotePeer.Close()
			log.Warn("[p2p]connecting peer %d ip format is wrcntm %s, close", version.P.Nonce, data.Addr)
			return
		}
		if ipNew == ipOld {
			//same id and same ip
