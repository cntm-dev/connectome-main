...
import (
	...
	netreqactor "github.com/cntmio/cntmology/p2pserver/actor/req"
...
// TestPingHandle tests Function PingHandle handling a pcntm message
func TestPcntmHandle(t *testing.T) {
	// Simulate a remote peer to be added to the neighbor peers
	var testID uint64
	_, testPub, _ := keypair.GenerateKeyPair(keypair.PK_ECDSA, keypair.P256)
	key := keypair.SerializePublicKey(testPub)
	...
	// Invoke PingHandle to handle the msg
	err = PcntmHandle(msg, network, nil)
	assert.Nil(t, err)
