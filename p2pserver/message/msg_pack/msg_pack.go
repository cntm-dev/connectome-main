...
func NewPcntmMsg(height uint64) ([]byte, error) {
	...
	tmpBuffer := bytes.NewBuffer([]byte{})
	serialization.WriteUint64(tmpBuffer, pcntm.Height)
	b := new(bytes.Buffer)
	err := binary.Write(b, binary.LittleEndian, tmpBuffer.Bytes())
	if err != nil {
		log.Error("Binary Write failed at new Msg")
	...
	checkSumBuf := mt.CheckSum(b.Bytes())
	pcntm.Init("pcntm", checkSumBuf, uint32(len(b.Bytes())))
	log.Debug("NewPcntmMsg The message payload length is ", pcntm.Length)
