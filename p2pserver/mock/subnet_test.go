...
func NewSubnetNode(acct *account.Account, listenAddr string, seeds, govs []string, net Network, reservedPeers []string,
	...
	info := peer.NewPeerInfo(seedId.Id, 0, 0, true, 0,
		0, 0, "v2.0.0", "")
	ccntmext := fmt.Sprintf("peer %s-%s: ", logPrefix, seedId.Id.ToHexString()[:6])
	logger := common.LoggerWithCcntmext(common.NewGlobalLoggerWrapper(), ccntmext)
...
func (self *TestSubnetProtocalHandler) HandlePeerMessage(ctx *p2p.Ccntmext, msg msgTypes.Message) {
