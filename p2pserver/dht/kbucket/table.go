...
// CplRefresh ccntmains a CPL(common prefix length) with the host & the last time
...
// Update adds or moves the given peer to the frcntm of its respective bucket
...
func (rt *RouteTable) nextBucket() {
	// This is the last bucket, which allegedly is a mixed bag ccntmaining peers not belcntming in dedicated (unfolded) buckets.
	// _allegedly_ is used here to denote that *all* peers in the last bucket might feasibly belcntm to another bucket.
	// This could happen if e.g. we've unfolded 4 buckets, and all peers in folded bucket 5 really belcntm in bucket 8.
	bucket := rt.Buckets[len(rt.Buckets)-1]
	newBucket := bucket.Split(len(rt.Buckets)-1, rt.local)
	rt.Buckets = append(rt.Buckets, newBucket)

	// The newly formed bucket still ccntmains too many peers. We probably just unfolded a empty bucket.
