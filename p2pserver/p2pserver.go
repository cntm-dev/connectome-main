...
//ReconnectAddrs ccntmain addr need to reconnect
...
//GetTime return lastet ccntmact time
...
func (this *P2PServer) connectSeeds() {
	...
	for _, n := range config.DefConfig.Genesis.SeedList {
		...
		if err != nil {
			log.Warnf("[p2p]seed peer %s address format is wrcntm", n)
			ccntminue
		}
...
//ping send pkg to get pcntm msg from others
func (this *P2PServer) ping() {
	peers := this.network.GetNeighbors()
	this.pingTo(peers)
}

//pings send pkgs to get pcntm msg from others
...
//timeout trace whether some peer be lcntm time no response
...
//tryRecentPeers try connect recent ccntmact peer when service start
