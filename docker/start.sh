...
set -e
# ensure the cntmology wallet.dat
if [ ! -f /app/${NODE}.dat ]; then
    if [[ "$NODE" == "NODE1" ]]; then
        for i in {1..7}
        do
            ./cntmology account add -d -w NODE${i}.dat <<EOF
server
server
EOF
...
echo "server" | ./cntmology --config=/data/config.json --wallet /data/wallet.dat --data-dir /data/Chain --enable-consensus --loglevel=${LOG_LEVEL} --networkid=${NETWORKID} --disable-log-file
