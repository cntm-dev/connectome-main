...
func (rt *restServer) registryMethod() {
	...
	getMethodMap := map[string]Action{
		...
		Api_GetCcntmract:         {name: "getccntmract", handler: GetCcntmract},
...
func (rt *restServer) getPath(url string) string {
	...
	if strings.Ccntmains(url, strings.TrimRight(Api_GetblockTxsByHeight, ":height")) {
		return Api_GetblockTxsByHeight
	} else if strings.Ccntmains(url, strings.TrimRight(Api_Getblockbyheight, ":height")) {
		return Api_Getblockbyheight
	} else if strings.Ccntmains(url, strings.TrimRight(Api_Getblockhash, ":height")) {
		return Api_Getblockhash
	} else if strings.Ccntmains(url, strings.TrimRight(Api_Getblockbyhash, ":hash")) {
		return Api_Getblockbyhash
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetTotalIssued, ":assetid")) {
	...
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetBalancebyAsset, ":addr/:assetid")) {
		return Api_GetBalancebyAsset
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetUTXObyAddr, ":addr")) {
		return Api_GetUTXObyAddr
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetUTXObyAsset, ":addr/:assetid")) {
		return Api_GetUTXObyAsset
	} else if strings.Ccntmains(url, strings.TrimRight(Api_Getasset, ":hash")) {
		return Api_Getasset
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetStateUpdate, ":namespace/:key")) {
		return Api_GetStateUpdate
	} else if strings.Ccntmains(url, strings.TrimRight(Api_GetSmartCodeResults, ":height")) {
...
func (rt *restServer) Stop() {
	if rt.server != nil {
		rt.server.Shutdown(ccntmext.Background())
