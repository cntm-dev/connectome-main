...
func (cp *ConsensusPayload) GetProgramHashes() ([]common.Uint160, error) {
	...
	ccntmract, err := ccntmract.CreateSignatureCcntmract(miners[cp.MinerIndex])
	hash := ccntmract.ProgramHash
	fmt.Println("program hash== ", hash)

	//signatureRedeemScript, err := ccntmract.CreateSignatureRedeemScript(miners[cp.MinerIndex])
	if err != nil {
