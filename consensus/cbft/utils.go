...
import (
	...
	gov "github.com/conntectome/cntm/smartcontract/service/native/governance"
...
func GetCbftConfigInfo(memdb *overlaydb.MemDB) (*config.CbftConfig, error) {
	//get governance view
	...
	chainconfig := new(config.CbftConfig)
	if preCfg.SetView == goveranceview.View {
		chainconfig = &config.CbftConfig{
		...
		data, err := GetStorageValue(memdb, ledger.DefLedger, nutils.GovernanceContractAddress, []byte(gov.Cbft_CONFIG))
		if err != nil {
			return nil, err
		}
...
func GetPeersConfig(memdb *overlaydb.MemDB) ([]*config.CbftPeerStakeInfo, error) {
	goveranceview, err := GetGovernanceView(memdb)
	if err != nil {
		return nil, err
	}
...
func getChainConfig(memdb *overlaydb.MemDB, blkNum uint32) (*vconfig.ChainConfig, error) {
	config, err := GetCbftConfigInfo(memdb)
	if err != nil {
