...
import (
	...
	p2pmsg "github.com/conntectome/cntm/p2pserver/message/types"
	gover "github.com/conntectome/cntm/smartcontract/service/native/governance"
...
func NewCbftServer(account *account.Account, txpool, p2p *actor.PID) (*Server, error) {
	server := &Server{
	...
	pid, err := actor.SpawnNamed(props, "consensus_Cbft")
	if err != nil {
		return nil, err
	}
	server.pid = pid
	...
	if err := server.initialize(); err != nil {
		return nil, fmt.Errorf("Cbft server start failed: %s", err)
	}
	return server, nil
}
...
func (self *Server) Receive(context actor.Context) {
	...
	case *actor.Restarting:
		log.Info("Cbft actor restarting")
	case *actor.Stopping:
		log.Info("Cbft actor stopping")
	case *actor.Stopped:
		log.Info("Cbft actor stopped")
	case *actor.Started:
		log.Info("Cbft actor started")
	case *actor.Restart:
		log.Info("Cbft actor restart")
	case *actorTypes.StartConsensus:
		log.Info("Cbft actor start consensus")
	case *actorTypes.StopConsensus:
		self.stop()
	case *message.SaveBlockCompleteMsg:
		log.Infof("Cbft actor SaveBlockCompleteMsg receives block complete event. block height=%d, numtx=%d",
	...
	case *message.BlockConsensusComplete:
		log.Infof("Cbft actor  BlockConsensusComplete receives block complete event. block height=%d, numtx=%d",
			msg.Block.Header.Height, len(msg.Block.Transactions))
		self.handleBlockPersistCompleted(msg.Block)
	...
	default:
		log.Info("Cbft actor: Unknown msg ", msg, "type", reflect.TypeOf(msg))
	}
}
...
func (self *Server) run(peerPubKey keypair.PublicKey) error {
	...
	go func() {
		for {
			...
			msg, err := DeserializeCbftMsg(msgData)

			if err != nil {
				log.Errorf("server %d failed to deserialize Cbft msg (len %d): %s", self.Index, len(msgData), err)
			} else {
...
func (self *Server) msgSendLoop() {
	...
	for {
		...
		case evt := <-self.msgSendC:
			...
			payload, err := SerializeCbftMsg(evt.Msg)
			if err != nil {
