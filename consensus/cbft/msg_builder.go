...
func DeserializeCbftMsg(msgPayload []byte) (ConsensusMsg, error) {
...
func SerializeCbftMsg(msg ConsensusMsg) ([]byte, error) {
...
func (self *Server) constructProposalMsg(blkNum uint32, sysTxs, userTxs []*types.Transaction, chainconfig *vconfig.ChainConfig) (*blockProposalMsg, error) {
	...
	CbftBlkInfo := &vconfig.CbftBlockInfo{
		Proposer:           self.Index,
		VrfValue:           vrfValue,
		VrfProof:           vrfProof,
	...
	consensusPayload, err := json.Marshal(CbftBlkInfo)
	if err != nil {
		return nil, err
	}
