...
func deepCopy(peersInfo []*config.CbftPeerStakeInfo) ([]*config.CbftPeerStakeInfo, error) {
	var peers []*config.CbftPeerStakeInfo
...
func genConsensusPayload(cfg *config.CbftConfig, txhash common.Uint256, height uint32) ([]byte, error) {
	if cfg.C == 0 {
	...
	return json.Marshal(CbftBlockInfo)
}

//GenesisChainConfig return chainconfig
func GenesisChainConfig(conf *config.CbftConfig, peers []*config.CbftPeerStakeInfo, txhash common.Uint256, height uint32) (*ChainConfig, error) {
...
func GenesisConsensusPayload(txhash common.Uint256, height uint32) ([]byte, error) {
	...
	case "Cbft":
		return genConsensusPayload(config.DefConfig.Genesis.Cbft, txhash, height)
	}
	return nil, nil
}
