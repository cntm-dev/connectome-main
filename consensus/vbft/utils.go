...
import (
	...
	"github.com/cntmio/cntmology-crypto/vrf"
...
func GetVbftConfigInfo(memdb *overlaydb.MemDB) (*config.VBFTConfig, error) {
	...
	data, err := GetStorageValue(memdb, ledger.DefLedger, nutils.GovernanceCcntmractAddress, []byte(gov.PRE_CONFIG))
	...
	if preCfg.SetView == goveranceview.View {
		...
		data, err := GetStorageValue(memdb, ledger.DefLedger, nutils.GovernanceCcntmractAddress, []byte(gov.VBFT_CONFIG))
		if err != nil {
			return nil, err
		}
...
func GetPeersConfig(memdb *overlaydb.MemDB) ([]*config.VBFTPeerStakeInfo, error) {
	...
	data, err := GetStorageValue(memdb, ledger.DefLedger, nutils.GovernanceCcntmractAddress, key)
	if err != nil {
		return nil, err
	}
...
func GetGovernanceView(memdb *overlaydb.MemDB) (*gov.GovernanceView, error) {
	value, err := GetStorageValue(memdb, ledger.DefLedger, nutils.GovernanceCcntmractAddress, []byte(gov.GOVERNANCE_VIEW))
	if err != nil {
		return nil, err
	}
