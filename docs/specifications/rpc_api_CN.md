...
| [getversion](#9-getversion) |  | 得到运行的cntmology版本 |  |
| [getccntmractstate](#10-getccntmractstate) | script_hash,[verbose] | 根据合约地址，得到合约信息 |  |
...
| [getbalance](#15-getbalance) | address | 返回base58地址的余额, cntm精度0，cntm精度9 |  |
...
| [getunboundcntm](#19-getunboundcntm) | address | 返回该账户未提取的cntm |  |
| [getblocktxsbyheight](#20-getblocktxsbyheight) | height | 返回该高度对应的区块落账的交易的哈希 |  |
| [getnetworkid](#21-getnetworkid) |  | 获取 network id |  |
| [getgrantcntm](#22-getgrantcntm) |  | 获取 grant cntm |  |
| [getsyncstatus](#23-getsyncstatus) |  | 获取 节点同步的状态 |  |
| [getbalancev2](#24-getbalancev2) | address | 返回base58地址的余额, cntm精度9， cntm精度18 |  |
| [getallowancev2](#25-getallowancev2) | asset, from, to | 返回允许从from转出到to账户的额度,cntm精度9， cntm精度18 |  |
...
Hex: 签名后的交易序列化成的十六进数据。可以参考cntmology-go-sdk/rpc.go中的NewNativeInvokeTransaction方法生成。
...
返回base58地址的余额, cntm精度9， cntm精度18
...
返回允许从from转出到to账户的额度, cntm精度9, cntm精度18
