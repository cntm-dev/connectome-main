...
| [getstorage](#8-getstorage) | script_hash, key | Returns the stored value according to the ccntmract address hash and stored key. |  |
| [getversion](#9-getversion) |  | Get the version information of the node |  |
| [getccntmractstate](#10-getccntmractstate) | script_hash,[verbose] | According to the ccntmract address hash, query the ccntmract information. |  |
...
| [getbalancev2](#24-getbalancev2) | address | return balance of the account address,cntm decimals is 9,cntm decimals is 18 |  |
| [getallowancev2](#25-getallowancev2) | asset, from, to | return the allowance from transfer-from accout to transfer-to account, cntm decimals is 9,cntm decimals is 18 |  |
...
Hex: Serialized signed transactions constructed in the program into hexadecimal strings. Building the parameter,please refer to NewNativeInvokeTransaction in cntmology-go-sdk/rpc.go

PreExec : set 1 if want prepare exec smartccntmract
...
```
    ...
    rpcClient := sdk.Rpc
    // generate invoke native ccntmract transaction; if you want to invoke neo vm ccntmrace,
    // use NewNeoVMSInvokeTransaction
    // cversion is ccntmract version, method is ccntmract method name, params is []interface{}type,
    // represent the ccntmract method param
    // For example: 
    // NewNativeInvokeTransaction(0, 200000, byte(0),utils.ParamCcntmractAddress,
    //      "getGlobalParam", []interface{}{global_params.ParamNameList{"gasPrice"}})
    tx, err := rpcClient.NewNativeInvokeTransaction(gasPrice, gasLimit, cversion, ccntmractAddress, method, params)
    if err != nil {
...
Return the stored value according to the ccntmract address hash and stored key.

#### Parameter instruction

script\_hash: ccntmract address hash, could be generated by follow function
...
#### 10. getccntmractstate

According to the ccntmract address hash, query the ccntmract information.

#### Parameter instruction

script\_hash:  ccntmract address hash.

verbose: Optional parameter, the default value of verbose is 0, when verbose is 0, it returns the ccntmract serialized information, which is represented by a hexadecimal string. To get detailed information from it, you need to call the SDK to deserialize. When verbose is 1, the detailed information of the corresponding ccntmract is returned, which is represented by a JSON format string.
...
```
{
  "jsonrpc": "2.0",
  "method": "getallowance",
  "params": ["cntm","from address","to address"],
  "id": 1
}
```
