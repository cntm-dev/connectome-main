...
| NextBookkeeper | Address | Accounting ccntmract hash value for the next block |
...
| getstorage | script_hash | Returns the stored value according to the ccntmract script hashes and stored key. |  |
| getversion |  | Get the version information of the node |  |
| getccntmractstate | script_hash,[verbose] | According to the ccntmract script hash, query the ccntmract information. |  |
...
| :--- | :--- | :--- | :--- |
| Hash/height | String/lcntm | Required | Block hash/height |
...
Hex: Serialized signed transactions constructed in the program into hexadecimal strings.Building the parameter,please refer to TestInvokefunction in cntmology/http/func_test.go.
...
script\_hash: Ccntmract script hash.
