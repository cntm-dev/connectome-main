...
* [Ontology Signature Server Tutorials](#cntmology-signature-server-tutorials)
	...
	* [2. Signature Service Method](#2-signature-service-method)
		...
		* [2.6 Native Ccntmract Invokes Signature](#26-native-ccntmract-invokes-signature)
			...
			* [Example2: Constructing  withdraw cntm transaction](#example2-constructing-withdraw-cntm-transaction)
...
The signature service currently supports signature for data, single signature and multi-signatures for raw transactions, constructing cntm/cntm transfer transactions and signing, constructing transactions that Native ccntmracts can invoke and signing, and constructing transactions that NeoVM ccntmracts can invoke and signing, and so on.
...
```
{
    ...
    "asset":"cntm",    //asset: cntm or cntm
    "from":"XXX",     //Payment account
    "to":"XXX",       //Receipt address
    "amount":"XXX"    //transfer amount. Note that since the precision of cntm is 9, it is necessary to multiply the actual transfer amount by 1000000000 when making cntm transfer.
}
```
Response result:
...
### 2.6 Native Ccntmract Invokes Signature

The Native ccntmract invocation transaction is constructed and signed according to the ABI.


Note:
When sigsvr starts, it will default seek the native ccntmract abi under "./abi" in the current directory. If there is no abi for this ccntmract in the naitve directory, then it will return a 1007 error. Can use --abi parameter to change the abi seeking path.
...
```
{
    "gas_price":XXX,    //gasprice
    "gas_limit":XXX,    //gaslimit
    "address":"XXX",    //The address that invokes native ccntmract
    "method":"XXX",     //The method that invokes native ccntmract
    "version":0,        //The version that invokes native ccntmract
    "params":[
        //The parameters of the Native ccntmract are constructed according to the ABI of calling method. All values ​​are string type.
    ]
}
```
Response result:
...
```
{
    "gas_price":XXX,    //gasprice
    "gas_limit":XXX,    //gaslimit
    "address":"XXX",    //The address that invokes NeoVM ccntmract
    "payer":"XXX",      //The fee payer's account address
    "params":[
        //The parameters of the Native ccntmract. All values are string type.
    ]
}
```
#### Example2: Constructing withdraw cntm transaction
...
``` json
{
	...
	"Params":{
		...
		"params":[
			...
			"AFmseVrdL9f9oyCzZefL9tG6UbvhUMqNMV",   //cntm ccntmract address (in base58 style)
			"ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48",  //cntm accept address. Note that accept address  can different with withdraw address
			"310860000000000"												//withdraw cntm amount. Note that cntm has 9 decimals
		]
	}
}
```

### 2.7 NeoVM Ccntmract Invokes Signature

The NeoVM parameter ccntmract supports array, bytearray, string, int, and bool types. When constructing parameters, it is necessary to provide parameter types and parameter values. The parameter values use string types. Array is an array of objects and supports all types and quantities of NeoVM supported parameters.
...
### 2.8 NeoVM Ccntmract Invokes By ABI Signature

NeoVM ccntmract invoke by abi transaction is constructed and signed according to the ABI, need the ABI of ccntmract and invoke parameters.
...
```
{
    "gas_price":XXX,    //gasprice
    "gas_limit":XXX,    //gaslimit
    "address":"XXX",    //The NeoVM ccntmract address
    "params":[XXX],     //The parameters of the NeoVM ccntmract are constructed according to the ABI of calling method. All values are string type.
    "ccntmract_abi":XXX, //The ABI of ccntmract
}
```
Response result:
```
...
Note than in order wcntm lose the account created by sigsvr, please backup wallet data on time, and backup sigsvr log.
