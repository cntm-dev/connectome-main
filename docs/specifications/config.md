...
SeedList：This is used to configure seed node of cntmology network。Seed node is the link entrance of cntmology network. When a new node link to 
cntmology network, it request network information from seed node. At least one seed node should be configured in the configuration file.
...
VBFTis the default consensus mechanism for cntmology at present, with [`config-vbft.json`](config-vbft.json) configuration files, as follows:
...
- n：the number of node in whole cntmology network;
...
- admin_cntm_id：cntm ID of administrator;
- min_init_stake：the minimum amount of initial stack cntm;
...
- peers：bookkeeper in consensus nodes configuration, as follows：
	...
	- initPos：initial mortgage cntm number of this bookkeeper.
