...
* [Ontology cli 使用说明](#cntmology-cli-使用说明)
	* [1、启动和管理Ontology节点](#1-启动和管理cntmology节点)
		* [1.1 启动参数](#11-启动参数)
			* [1.1.1 Ontology 系统参数](#111-cntmology-系统参数)
			* [1.1.2 账户参数](#112-账户参数)
			* [1.1.3 共识参数](#113-共识参数)
	...
	* [3、资产管理](#3-资产管理)
		...
		* [3.6 查看未解绑的cntm余额](#36-查看未解绑的cntm余额)
		* [3.7 提取解绑的cntm](#37-提取解绑的cntm)
			* [3.7.1 提取解绑的cntm参数](#371-提取解绑的cntm参数)
	* [4、查询信息](#4-查询信息)
	...
	* [7、构造交易](#7-构造交易)
		...
		* [7.4 构造提取cntm交易](#74-构造提取cntm交易)
			* [7.4.1 构造提取cntm交易参数](#741-构造提取cntm交易参数)
	* [8、对交易签名](#8-对交易签名)
...
./cntmology
```
使用./cntmology -help 可以查看到Ontology cli节点支持的所有启动参数。
...
```
./cntmology --enable-consensus --disable-rpc --disable-event-log
```
...
使用 ./cntmology account --help 命令可以查看钱包管理命令的帮助信息。
...
每个钱包都一个默认账户，一般情况下是第一个添加的账户。默认账户不能被删除，可以通过./cntmology account set 命令来修改默认账户。
...
--cntmid
cntmid参数用来创建cntm ID，而不是普通账户。

**添加账户**

```
./cntmology account add --default
```

通过 ./cntmology account add --help 可以查看帮助信息。
...
使用--v 可以查看账户的详细信息。
通过 ./cntmology account list --help 可以查看帮助信息。

### 2.3 修改账户

使用修改账户命令可以修改账户的标签，重新设置默认账户，修改账户密码，如果账户是ECDSA加密算法的密钥，还可以修改密钥的签名方案。
通过 ./cntmology account set --help 可以查看帮助信息。
...
```
./cntmology account set --d <address|index|label>
```
**修改账户标签**

```
./cntmology account set --label=XXX <address|index|label>
```
**修改账户密码**

```
./cntmology account set --changepasswd <address|index|label>
```

**修改ECDSA密钥签名方案**

```
./cntmology account set --s=SHA256withECDSA <address|index|label>
```
### 2.4 删除账户
...
```
/cntmology account del <address|index|label>
```
### 2.5 导入账户
...
```
./cntmology account import -s=./source_wallet.dat
```

#### 2.5.2 通过WIF导入账户
获得WIF并把WIF存入key.txt文件，并通过以下命令导入
cntmology account import --wif --source key.txt

## 3、资产管理

资产管理命令可以查看账户的余额，执行cntm/cntm转账，提取cntm以及查看未绑定的cntm等操作。
...
--gaslimit
gaslimit参数指定转账交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支付的cntm 费用。 默认值为20000。

--asset
asset参数指定转账的资产类型，cntm表示cntm，cntm表示cntm。默认值为cntm。
...
--amount
amount参数指定转账金额。注意：由于cntm的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；cntm的精度为9，因此超出9位的小数部分将会被丢弃。
...
```
./cntmology asset transfer --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=cntm
```
...
--amount
amount参数指定授权转账金额。注意：由于cntm的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；cntm的精度为9，因此超出9位的小数部分将会被丢弃。

**授权转账**

```
./cntmology asset approve --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=cntm
```
...
```
./cntmology asset allowance --from=<address|index|label> --to=<address|index|label>
```
...
--amount
amount参数指定转账金额，转账金额不能大于授权转账余额，否则交易会执行失败。注意：由于cntm的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；cntm的精度为9，因此超出9位的小数部分将会被丢弃。

**从授权账户中转账**

```
./cntmology asset transferfrom --from=<address|index|label> --to=<address|index|label> --sender=<address|index|label> --amount=XXX
```

### 3.6 查看未解绑的cntm余额

cntm采用定时解绑策略解除绑定在cntm上的cntm。使用如下命令可以查看到当前账户未解绑的cntm余额。

```
./cntmology asset unboundcntm <address|index|label>
```
### 3.7 提取解绑的cntm

使用提取命令可以提取当前所有未解绑的cntm。
...
**提取解绑的cntm**
```
./cntmology asset withdrawcntm <address|index|label>
```
## 4、查询信息

查询信息命令可以查询区块、交易以及交易执行等信息。使用./cntmology info block --help 命令可以查看帮助信息。

### 4.1 查询区块信息

```
./cntmology info block <height|blockHash>
```
...
```
其中，State表示交易执行结果，State的值为1，表示交易执行成功，State值为0时，表示执行失败。GasConsumed表示交易执行消耗的cntm。Notify表示交易执行时输出的Event log。不同的交易可能会输出不同的Event log。
...
智能部署前需要把在NeoVM合约编译器如：[SmartX](http://smartx.cntm.io) 上编译好的Code，保存在本地的一个文本文件中。
...
--gaslimit
gaslimit参数指定部署合约交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支付的cntm 费用。

**对于合约部署，gaslimit 值必须大于20000000，同时账户中必须保有足够的cntm余额。**
...
--prepare, -p
prepare参数用于预部署合约, 预部署不会把合约部署到Ontology上， 也不会消耗人任何cntm。通过预部署合约，用户可以知道当前合约部署所需要消耗的gas limit。

**智能合约部署**

```
./cntmology ccntmract deploy --name=xxx --code=xxx --author=xxx --desc=xxx --email=xxx --needstore --gaslimit=100000000
```
...
```
Deploy ccntmract:
  Ccntmract Address:806fbee1fcfb554af47844edd4d4ce2918737747
  TxHash:99d719f51837acfa48f9cd2a21983fb993bc8d5a763b497802f7b872be2338fe
```

可以通过 ./cntmology info status <TxHash> 命令查询合约执行状态。如果返回错误如：UNKNOWN TRANSACTION时，表示交易没有落帐，有可能交易还在交易池中排队等待被打包，也有可能表示交易因为gaslimit或者时gasprice设置过低，导致交易被拒绝。
...
Ccntmract Address为根据合约Code生成的合约地址。
...
--prepare, -p
prepare参数表示当前为预执行，执行交易不会被打包到区块中，也不会消耗任何cntm。预执行会返回合约方法的返回值，同时还会试算当前调用需要的gas limit。
...
```
./cntmology ccntmract invoke --address=XXX --params=XXX --return=XXX --p
```
返回示例：

```
Ccntmract invoke successfully
Gas consumed:20000
Return:0
```
**智能合约执行**
...
```
./cntmology ccntmract invoke --address=XXX --params=XXX --gaslimit=XXX
```

智能合约在执行之前，可以通过预执行，试算出当前执行所需要的gas limit，以避免cntm余额不足导致执行失败。
...
```
./cntmology ccntmract invokeCode --code=XXX --gaslimit=XXX
```
...
```
./cntmology import --importfile=./OntBlocks.dat
```
...
--amount
amount参数用于设定转账金额。浮点类型，如果0.001个cntm

--wallet
wallet 钱包路径。wallet参数用于解析账户的索引或者标签。如果账户使用的都是地址，则不需要此参数。

```
./cntmology buildtx transfer --from=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 --to=AaCe8nVkMRABnp5YgEjYZ9E5KYCxks2uce --amount=10
```
...
```
./cntmology buildtx approve  --from=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 --to=AaCe8nVkMRABnp5YgEjYZ9E5KYCxks2uce --amount=10
```
返回如下：
...
--amount
amount参数用于设定转账金额。浮点类型，如0.001个cntm

--wallet
wallet 钱包路径。wallet参数用于解析账户的索引或者标签。如果账户使用的都是地址，则不需要此参数。

```
./cntmology buildtx transferfrom --sender=AMFrW7hrSRw1Azz6hQohni8BdStZDvectW --from=Aaxjf7utmjSstmTD1LjtYfhZ3CoWaxC7Tt --to=AMFrW7hrSRw1Azz6hQohni8BdStZDvectW --amount=10
```
...
--receive
receive 参数用于设定提取的cntm接收账户。如果不设置，默认使用cntm提取账户。

--amount
amount参数用于设定提取的cntm金额。浮点类型，如0.001个cntm。如果不填，默认提取该账户下的所有可以提取cntm。
...
```
./cntmology buildtx withdrawcntm ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48
```
...
Withdraw cntm amount:2321499191858975
Withdraw raw tx:
...
```
./cntmology sigtx --account=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 
```
...
```
./cntmology multisigaddr --pubkey=03c0c30f11c7fc1396e8595bf2e339d553d728ea6f21ae831e8ab704ca14fe8a56,02b2b9fb60a0add9ef6715ffbac8bc7e81cb47cd06c157c19e6a858859c0158231 -m=1
```
返回如下：
...
```
./cntmology sendtx 
```
...
Tip:
  Using './cntmology info status f8ea91da985af249e808913b6398150079cdfb02273146e4eb69c43947a42db2' to query transaction status.
```
...
```
./cntmology showtx 
```
返回如下：
