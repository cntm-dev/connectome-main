...
--gaslimit
gaslimit参数指定转账交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支付的cntm 费用。 默认值为30000。
...
**对于合约部署，gaslimit 值必须大于10000000，同时账户中必须保有足够的cntm余额。**
