# Ontology EVM Ccntmracts

This doc aims at guiding developers to develop, deploy and test EVM smart ccntmracts on Ontology.

Now the Ontology network is compatible with Ethereum through Ontology EVM, meaning that Ontology supports EVM ccntmracts and Web3 JSON-RPC protocol. It's possible to use EVM tools like Truffle, Remix and Web3.js directly on Ontology.


* [1 Development Environment and Tools](#1-development-environment-and-tools)
  * [1.1 Remix](#11-remix)
    * [1.1.1 Initialize Remix](#111-initialize-remix)
    * [1.1.2 Compile Ccntmract](#112-compile-ccntmract)
    * [1.1.3 Deploy Ccntmract](#113-deploy-ccntmract)
    * [1.1.4 Invoke Ccntmract](#114-invoke-ccntmract)
  * [1.2 Truffle](#12-truffle)
    ...
    * [1.2.3 Deploy Ccntmract](#123-deploy-ccntmract)
  * [1.3 Hardhat](#13-hardhat)
    ...
    * [1.3.3 Deploy Ccntmract](#133-deploy-ccntmract)
* [2 Network Info](#2-network-info)
    * [2.1 Network Types](#21-network-types)
    * [2.2 EVM Assets on Ontology](#22-evm-assets-on-cntmology)
...
* [3 Key Management with MetaMask](#3-key-management-with-metamask)
    ...
    * [3.3 Initialize Ccntmract](#33-initialize-ccntmract)
    * [3.4 Call Functions](#34-Call-Functions)
* [4 Move Assets from Ethereum to Ontology](#4-move-assets-from-ethereum-to-cntmology)
* [5 Ccntmract Development](#5-ccntmract-development)
  * [5.1 Set up Environment](#51-set-up-environment)
  * [5.2 Ccntmract Design](#52-ccntmract-design)
    * [5.2.1 Ccntmract Logic](#521-ccntmract-logic)
    * [5.2.2 Define Ccntmract Event](#522-define-ccntmract-event)
    * [5.2.3 Define Function](#523-define-function)
  * [5.3 Compile and Test Ccntmract using Hardhat ](#53-compile-and-test-ccntmract-using-hardhat )
    ...
    * [5.3.5 Compile Ccntmract](#535-compile-ccntmract)
    * [5.3.6 Test Ccntmract](#536-test-ccntmract)
* [6 API Reference](#6-api-reference)

## 1 Development Environment and Tools

EVM smart ccntmracts are written using [Solidity](https://docs.soliditylang.org/en/v0.8.6/). You can reuse existing Ethereum ccntmract frameworks to develop and deploy EVM ccntmracts. 

### 1.1 Remix

[Remix IDE](https://remix.ethereum.org/#optimize=false&runs=200&evmVersion=null&version=soljson-v0.8.1+commit.df193b15.js) is an open source development environment for EVM ccntmracts. Remix IDE documentation is [here](https://remix-ide.readthedocs.io/en/latest/).

We will now go through an example of a Hello World ccntmract development using Remix.
...
Then, select Solidity environment. Create a new file and name it HelloWorld.sol. Then copy the code of [Hello World ccntmract](../ccntmract-demo/helloworlddemo/helloworld.sol) and paste it in the file just created.

![image-20210526143301031](./image-20210526143301031.png)

#### 1.1.2 Compile Ccntmract

Click on the Solidity Compiler button, select compiler version to 0.5.10 and start compiling HelloWorld.sol.

#### 1.1.3 Deploy Ccntmract

The ccntmract is ready to deploy on Ontology after compiling. Here we deploy it on Ontology testnet.

> **Notice: ** MetaMask must have been set up on Ontology before you deploy the ccntmract.

Select "Custom RPC" in MetaMask networks settings. Fill in and save the info below.
- Network name: cntmology testnet
- Node url："http://polaris1.cntm.io:20339" or "http://polaris2.cntm.io:20339" or "http://polaris3.cntm.io:20339"
- Chain ID：5851
- Blockchain explorer url："https://explorer.cntm.io/testnet"

![RemixIDE_Step1](./metamask_networks.png)

Next, apply for testnet cntm at Ontology [Faucet address](https://developer.cntm.io/) as gas fee.

Finally, select "Injected Web3" in Remix. Click "Deploy" to finish.

![deploy ccntmract](./remix_deploy.jpg)

#### 1.1.4 Invoke Ccntmract

Now you can call the method in this ccntmract. The string `hello` is saved in the ccntmract when you deploy it, you can call the method `message` to query this string:

![invoke ccntmract](./remix_invoke.jpg)

### 1.2 Truffle

Truffle offers tools and frameworks for EVM ccntmract development, testing and management. You can find more details [here](https://www.trufflesuite.com/docs/truffle/quickstart).

Now we will demonstrate how to use Truffle with this [test code](../ccntmract-demo/truffledemo).
...
module.exports = {
  networks: {
    cntmology: {
     provider: () => new HDWalletProvider(mnemonic, `http://polaris2.cntm.io:20339`),
     network_id: 5851,
...
#### 1.2.3 Deploy Ccntmract

Run this command to deploy the ccntmract on the Ontology network.

```
truffle migrate --network cntmology
```
...
```
Compiling your ccntmracts...
===========================
> Everything is up to date, there is nothing to compile.

Starting migrations...
======================
> Network name:    'cntmology'
> Network id:      12345
...
======================
   ...
   > Blocks: 0            Seconds: 4
   > ccntmract address:    0x53e137A51CfD1E1b088E0d921eB5dBCF9cFa955E
   > block number:        6264
...
=====================
   ...
   > Blocks: 0            Seconds: 4
   > ccntmract address:    0xfbff9bd546B0e0D4b40f6f758847b70050d01b37
   > block number:        6266
...
hello ccntmract address: 0xfbff9bd546B0e0D4b40f6f758847b70050d01b37
...
Hardhat is an Ethereum development environment. We will use this [test code](../ccntmract-demo/hardhatdemo) as an example and demonstrate how to use Hardhat.
...
module.exports = {
    defaultNetwork: "cntmology_testnet",
    networks: {
        hardhat: {},
        cntmology_testnet: {
            url: "http://polaris2.cntm.io:20339",
            chainId: 5851,
            gasPrice:2500,
            gas:2000000,
            timeout:10000000,
...
#### 1.3.3 Deploy Ccntmract 

Run this command in root of the project directory to deploy the ccntmract on Ontology Chain: 

```
$ npx hardhat run scripts/sample-script.js --network cntmology_testnet
```

The result looks like this:

```
sss@sss hardhatdemo % npx hardhat run scripts/sample-script.js --network cntmology_testnet
...
| chainId        | 58                                                                                                                      |
| Gas Token      | cntm Token                                                                                                               |
| RPC            | http://dappnode1.cntm.io:20339,http://dappnode2.cntm.io:20339,http://dappnode3.cntm.io:20339,http://dappnode4.cntm.io:20339 |
| Block Explorer | https://explorer.cntm.io/                                                                                                |
...
| chainId        | 5851                                                                                                                  |
| Gas Token      | cntm Token                                                                                                             |
| RPC            | http://polaris1.cntm.io:20339, http://polaris2.cntm.io:20339, http://polaris3.cntm.io:20339,http://polaris4.cntm.io:20339 |
| Block Explorer | https://explorer.cntm.io/testnet                                                                                       |

### 2.2 EVM Assets on Ontology

| Name | Address                                     |
| :--- | :------------------------------------------ |
| cntm  | 0x00000000000000000000000000000000000000000 |

### 2.3 OEP-4 Assets

Please refer to this [link](https://explorer.cntm.io/tokens/oep4/10/1#).

### 2.4 Gas Fee

Ontology EVM ccntmracts consume cntm as gas fee for execution. You can apply for testnet cntm [here](https://developer.cntm.io/).
...
MetaMask is a non-custodial wallet. tTe user's private key is encoded with the mnemonic phrase and stored in user's local browser. Once lost, the user can no lcntmer ccntmrol the savings or restore access to the wallet. MetaMask communicates with Ethereum Ledger via Infura. Please refer to the [MetaMask website](https://metamask.io/) for more details.
...
Call the function:
   ...
   ```js
     getWeb3()
    .then((result) => {
        this.web3 = result;// we instantiate our ccntmract next
    });
   ```
...
### 3.3 Initialize Ccntmract

Initialize your ccntmract after completing above steps. 

### 3.4 Call Functions

Now you can call any function by directly interacting with the instantiated ccntmract. Please note that:

Functions that do not alter the state of the ccntmract are `call()` functions. Below is an example of calling a `call()` function: 

```js
  this.myCcntmractInstance.methods.myMethod(myParams)
    .call()
    .then(
...
Functions that alter the state of the ccntmract are `send()` functions. Below is an example of calling a `send()` function: 
...
## 5 Ccntmract Development

Now we will demonstrate the full process of ccntmract development, deployment and testing using Hardhat. 
...
### 5.2 Ccntmract Design 

#### 5.2.1 Ccntmract Logic

The ccntmract we use as an example here is for sending red packets, which is used when users send crypto assets as gifts. The core functions are:

- Send red packets
- Receive red packets

Before sending red packets, the user need to determine the amount of tokens to be sent and the number of red packets. For instance, 100 tokens will be sent in 10 red packets (to 10 different wallets). For ease of understanding, each red packet ccntmains the same amount, meaning that each ccntmains 10 tokens.
...
#### 5.2.2 Define Ccntmract Event

When executing the ccntmract, we can trace the process by adding events.

Here we design two events:

1. When the user send a red packet, the ccntmract generates an ID for the red packets, which will be sent through this event notification:
```
...
Sends red packets. Any system is able to call the function and send certain amount of tokens to the ccntmract address. Other addresses can receive red packets from this ccntmract address.

> **Notice:** Before invoking this function, the ccntmract has to be authorized to transfer tokens from users' addresses. To do so, call the `approve` method of the token first.
...
```
View the full code [here](../ccntmract-demo/hardhatdemo/ccntmracts/Redpacket.sol).

### 5.3 Compile and Test Ccntmract using Hardhat 
...
`accounts` field takes the array of selected private key. There should be enough cntm balance in the corresponding address to pay for transactions. Apply Testnet cntm [here](https://developer.cntm.io/).

#### 5.3.3 File Preparation

Add the ccntmract file in the `ccntmracts` folder. To support ERC-20 token transfer, we also need `EIP20Interface.sol`, `UniversalERC20.sol`, and `TokenDemo.sol` which you can download from [here](../ccntmract-demo/hardhatdemo/ccntmracts).
...
describe("RedPacket", function () {
    ...
    beforeEach(async function () {
        const TokenDemo = await ethers.getCcntmractFactory("TokenDemo");
        tokenDemo = await TokenDemo.deploy(10000000, "L Token", 18, "LT");
        await tokenDemo.deployed();
        const RedPacket = await ethers.getCcntmractFactory("RedPacket");
...
#### 5.3.5 Compile Ccntmract

Run this command in the root directory to compile the ccntmract.
...
| [eth_sendRawTransaction](#eth_sendrawtransaction)                                   | Creates new message call transaction or a ccntmract creation for signed transactions                         |
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"net_version","params":[],"id":3}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_chainId","params":[],"id":67}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":83}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getBalance","params":["0x407d73d8a49eeb85d32cf465507dd71d507100c1", "latest"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_syncing","params":[],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_gasPrice","params":[],"id":73}'
```
...
Calculating the correct position depends on the storage to retrieve. Consider the following ccntmract deployed at `0x295a70b2de5e3953354a6a8344e616ed314d7251` by address `0x391694e7e0b0cce554cb130d723a9d27458f9298`:
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0", "method": "eth_getStorageAt", "params": ["0x295a70b2de5e3953354a6a8344e616ed314d7251", "0x0", "latest"], "id": 1}' localhost:8545
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0", "method": "eth_getStorageAt", "params": ["0x295a70b2de5e3953354a6a8344e616ed314d7251", "0x6661e9d6d8b923d5bbaab1b96e1dd51ff6ea2a93520fdc9eb75d059238b8c5e9", "latest"], "id": 1}' localhost:8545
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getTransactionCount","params":["0x407d73d8a49eeb85d32cf465507dd71d507100c1","latest"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getBlockTransactionCountByHash","params":["0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getBlockTransactionCountByNumber","params":["0xe8"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getCode","params":["0xa94f5374fce5edbc8e2a8697c15331677e6ebf0b", "0x2"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{
     "jsonrpc": "2.0",
     "id": 2233,
...
Creates new message call transaction or a ccntmract creation for signed transactions.
...
Use [eth_getTransactionReceipt](#-eth_getTransactionReceipt) to get the ccntmract address, after the transaction was mined, when you created a ccntmract.

#### Request Example

```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_sendRawTransaction","params":[{see above}],"id":1}'
```
...
`DATA` - the return value of executed ccntmract.

#### Request Example

```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_call","params":[{see above}],"id":1}'
```
...
```shell
curl -X POST --data http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' '{"jsonrpc":"2.0","method":"eth_estimateGas","params":[{see above}],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["0x1b4", true],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getBlockByHash","params":["0xe670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d1527331", true],"id":1}'
```
...
- `to`: `DATA`, 20 Bytes - address of the receiver. null when its a ccntmract creation transaction.
- `value`: `QUANTITY`- value transferred in Wei.
- `gasPrice`: `QUANTITY` - gas price provided by the sender in Wei.
- `gas`: `QUANTITY` - gas provided by the sender.
- `input`: `DATA` - the data send alcntm with the transaction.

#### Request Example

```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getTransactionByHash","params":["0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getTransactionByBlockHashAndIndex","params":["0xc6ef2fc5426d6ad6fd9e2a26abeab0aa2411b7ab17f30a99d3cb96aed1d1055b", "0x0"],"id":1}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_getTransactionByBlockNumberAndIndex","params":["0x29c", "0x0"],"id":1}'
```
...
- `ccntmractAddress`: `DATA`, 20 Bytes - The ccntmract address created, if the transaction was a ccntmract creation, otherwise `null`.
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{    "jsonrpc": "2.0",
    "id": 16661,
...
```
{
    ...
    "result": {
        ...
        "blockNumber": "0x204",
        "ccntmractAddress": "0xddcb212ce4896bb02f79db726f6bb8588df41a5c",
