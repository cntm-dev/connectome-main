...
cntmology目前已支持EVM合约，开发者可以在cntmology链上部署和调用EVM合约, 并且支持ethereum相关的合约调用工具如web3js等。

* [Ontology支持EVM的介绍](#Ontology支持EVM的介绍)
    * [1 Developer Quick Start](#1-Developer-Quick-Start)
    * [2 部署EVM合约到cntmology链](#2-部署EVM合约到cntmology链)
        ...
        * [2.2 使用Truffle](#22-使用Truffle)
            ...
            * [2.2.3 部署合约到cntmology链](#223-部署合约到cntmology链)
        * [2.3 使用Hardhat](#23-使用Hardhat)
            * [2.3.1 搭建Hardhat开发环境](#231-搭建Hardhat开发环境)
            * [2.3.2 配置hardhat-config](#232-配置hardhat-config)
            * [2.3.3 部署合约到cntmology链](#233-部署合约到cntmology链)
    * [3 网络详情](#3-网络详情)
        * [3.1 节点网络](#31-节点网络)
        * [3.2 cntmology链上EVM资产列表](#32-cntmology链上EVM资产列表)
    * [4 钱包使用](#4-钱包使用)
        * [4.1 Metamask](#41-Metamask)
            ...
            * [4.1.4 调用函数](#414-调用函数)
    * [5 ethereum链上的资产跨到cntmology链上](#5-ethereum链上的资产跨到cntmology链上)
    * [6 在cntmology链上开发一个新的EVM合约](#6-在cntmology链上开发一个新的EVM合约)
        * [6.1 环境准备](#61-环境准备)
...
cntmology目前已完全支持EVM合约，并且已支持ethereum链的节点调用方式，也就是说我们可以使用ethereum相关的合约开发工具在cntmology链上开发部署测试EVM合约，比如Truffle, Remix,
Web3js等工具。

## 2 部署EVM合约到cntmology链
...
`helloworld`源码请[参考](../ccntmract-demo/helloworlddemo/helloworld.sol)
...
- 选择自定义RPC网络，输入如下的配置信息，最后保存我们输入的配置。
    - 输入网络名 - "cntmology testnet"
    - 输入节点url - "http://polaris1.cntm.io:20339"或"http://polaris2.cntm.io:20339"或"http://polaris3.cntm.io:20339"
    - 输入Chain ID - 5851
    - 输入区块链浏览器url - "https://explorer.cntm.io/testnet"
- 去本体[Faucet地址](https://developer.cntm.io/)领取cntm。
...
完整的测试代码在[这里](../ccntmract-demo/truffledemo)
...
领取测试币[cntm](https://developer.cntm.io/)
...
## 5 ethereum链上的资产跨到cntmology链上

[PolyBridge](https://bridge.poly.network/)

## 6 在cntmology链上开发一个新的EVM合约

这部分我们会使用`hardhat`工具开发部署和测试EVM合约，并且会在本地起一个cntmology节点用于测试合约。
...
[合约完整的代码](../ccntmract-demo/hardhatdemo/ccntmracts/Redpacket.sol)
...
accounts字段指定的私钥数组，对应的地址需要有测试网的cntm,用于付交易的手续费，可以去去[这里](https://developer.cntm.io/)领取测试网cntm。

#### 6.3.3 红包合约

把之前的红包合约代码文件放到 `ccntmracts`文件夹下，为了支持ERC20的转账，我们还需要
`EIP20Interface.sol`, `UniversalERC20.sol`, 和 `TokenDemo.sol`
文件，可以从[此处](../ccntmract-demo/hardhatdemo/ccntmracts)下载相关文件
...
由于以太坊与本体交易的结构体和存储结构存在差异，目前本体只支持了以太坊部分RPC接口（cntmology部分接口返回的内容与以太坊返回的有所不同），具体如下：
