...
然后选择 Solidity 环境，创建新文件并命名为 HelloWorld.sol，再将已经写好的 [Hello World 合约](../ccntmract-demo/helloworlddemo/helloworld.sol)代码复制到该文件中。
...
- 网络名：cntmology testnet
- 节点url："http://polaris1.cntm.io:20339"或"http://polaris2.cntm.io:20339"或"http://polaris3.cntm.io:20339"
- 输入Chain ID：5851
- 输入区块链浏览器url："https://explorer.cntm.io/testnet"

![RemixIDE_Step1](./rpc.png)

之后需要在本体[Faucet地址](https://developer.cntm.io/)领取测试 cntm 作为手续费。
...
下面我们用这段[测试代码](../ccntmract-demo/truffledemo)作为范例介绍 Truffle 的使用。
...
Hardhat 是一个编译、部署、测试和调试以太坊应用的开发环境。下面我们用这段[测试代码](../ccntmract-demo/hardhatdemo)作为范例介绍 Hardhat 的使用。
...
请参考[此链接](https://explorer.cntm.io/tokens/oep4/10/1#)。

### 2.4 手续费

本体链上的 EVM 合约使用 cntm 作为手续费。可以在[此处](https://developer.cntm.io/)领取 cntm 测试币。
...
合约完整的代码请参考[此链接](../ccntmract-demo/hardhatdemo/ccntmracts/Redpacket.sol)。
...
`accounts`字段指定的私钥数组，对应的地址需要有测试网的cntm来支付交易的手续费，可以在[这里](https://developer.cntm.io/)领取测试网cntm。

#### 5.3.3 文件准备

把红包合约代码文件放到 `ccntmracts`文件夹下，为了支持 ERC-20 代币的转账，我们还需要`EIP20Interface.sol`, `UniversalERC20.sol`, 和 `TokenDemo.sol`文件，可以从[此处](../ccntmract-demo/hardhatdemo/ccntmracts)下载相关文件。
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_chainId","params":[],"id":83}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_protocolVersion","params":[],"id":67}'
```
...
```shell
curl -X POST http://127.0.0.1:20339 -H 'Ccntment-Type: application/json' --data '{"jsonrpc":"2.0","method":"eth_estimateGas","params":[{see above}],"id":1}'
```
...
- `ccntmractAddress`: `DATA`, 20字节 - 对于合约创建交易，该值为新创建的合约地址，否则为null
